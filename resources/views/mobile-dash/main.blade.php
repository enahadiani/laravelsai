...
<head>
    ...
    <style>
        ...
        .saicon {
            ...
            .icon-rp{
                background: #505050;
                -webkit-mask-image: url("{{ url('img/mobile-dash/rupiah.svg') }}");
                mask-image: url("{{ url('img/mobile-dash/rupiah.svg') }}");
                width: 20pt;
                height: 20pt;
            }
            .icon-notif{
                background: #505050;
                -webkit-mask-image: url("{{ url('img/mobile-dash/notif-outline.svg') }}");
                mask-image: url("{{ url('img/mobile-dash/notif-outline.svg') }}");
                width: 20pt;
                height: 20pt;
            }
            .icon-user{
                background: #505050;
                -webkit-mask-image: url("{{ url('img/mobile-dash/user-outline.svg') }}");
                mask-image: url("{{ url('img/mobile-dash/user-outline.svg') }}");
                width: 20pt;
                height: 20pt;
            }
    </style>
...
@endif
    ...
    function msgDialog(data){
        switch(data.type){
            ...
            case 'logout':
                ...
                //     window.location.href = "{{ url('mobile-dash/logout') }}";
                // }
    ...
    function updateNotifRead(){
        $.ajax({
            type: 'POST',
            url: "{{ url('mobile-dash/notif-update-status') }}",
            dataType: 'json',
            async:false,
            success:function(result){    
                $('.count').remove();
    ...
    function getJumlahNotRead(){
        $.ajax({
            type: 'GET',
            url: "{{ url('mobile-dash/jum-status-read') }}",
            dataType: 'json',
            async:false,
            success:function(result){    
    ...
    function loadForm(url){
        $.ajax({
            type: 'GET',
            url: "{{ url('mobile-dash/cek_session') }}",
            dataType: 'json',
            async:false,
            success:function(result){    
                if(!result.status){
    ...
    function getFormList() {
        $.ajax({
            type:'GET',
            url:"{{url('mobile-dash/search-form-list2')}}",
            dataType: 'json',
            async: false,
            success: function(result) {
                if(result.status) {
    ...
    function searchForm(cari){
        $.ajax({
            type: 'POST',
            url: "{{ url('mobile-dash/search-form') }}",
            dataType: 'json',
            data:{'cari':cari},
            async:false,
            success:function(result){  
    ...
    function loadProfile(){
        loadForm("{{url('mobile-dash/form/fProfile')}}");
    }
    ...
    $('.navbar_bottom').on('click','a',function(e){
        ...
        if(url != "" || url != "-"){
            loadForm("{{ url('mobile-dash/form') }}/"+url);
        }else{
            loadForm("{{ url('mobile-dash/form') }}/blankform");
        }
    });
    ...
    <script>
        ...
        $(window).scroll(function() {
            ...
            if($(window).scrollTop() == 0){
                $('#scroll-top').hide();
                $('#scroll-bottom').show();
            }else if( $(document).height() - ($(window).scrollTop() + $(window).height()) <= 100){
                $('#scroll-top').show();
                $('#scroll-bottom').hide();
