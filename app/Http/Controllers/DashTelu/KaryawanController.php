...
class KaryawanController extends Controller
{
    ...
    public function index(){
        try {
            $client = new Client();
            $response = $client->request('GET',  config('api.url').'ypt-master/karyawan',[
                'headers' => [
    ...
    public function store(Request $request) {
        ...
        try { 
            ...
            for($i=0;$i<count($name);$i++) {
                ...
                $client = new Client();
                $response = $client->request('POST',  config('api.url').'ypt-master/karyawan',[
                    'headers' => [
                        'Authorization' => 'Bearer '.Session::get('token'),
                        'Accept'     => 'application/json',
                    ],
    ...
    public function getData($id) {
        try{
            $client = new Client();
            $response = $client->request('GET',  config('api.url').'ypt-master/karyawan-detail?nik='.$id,
            [
                'headers' => [
                    'Authorization' => 'Bearer '.Session::get('token'),
                    'Accept'     => 'application/json',
    ...
    public function update(Request $request, $id) {
        ...
        try { 
            ...
            for($i=0;$i<count($name);$i++) {
                ...
                $client = new Client();
                $response = $client->request('POST',  config('api.url').'ypt-master/karyawan-ubah?nik='.$id,[
                    'headers' => [
                        'Authorization' => 'Bearer '.Session::get('token'),
                        'Accept'     => 'application/json',
                    ],
    ...
    public function delete($id) {
        try{
            $client = new Client();
            $response = $client->request('DELETE',  config('api.url').'ypt-master/karyawan?nik='.$id,
            [
                'headers' => [
                    'Authorization' => 'Bearer '.Session::get('token'),
                    'Accept'     => 'application/json',
