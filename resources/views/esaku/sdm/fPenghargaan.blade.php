...
{{-- LIST DATA --}}
<x-list-data judul="Data Penghargaan" tambah="" :thead="array('NIK','Nama','Jumlah Penghargaan','Aksi')" :thwidth="array(10,30,10,10)" :thclass="array('','','','text-center')" />
{{-- END LIST DATA --}}
                    ...
                    <div class="tab-content tabcontent-border col-12 p-0 mt-3">
                        <div class="tab-pane active" id="data-umum" role="tabpanel">
                            ...
                            <ul class="nav nav-tabs col-12 " role="tablist">
                                <li class="nav-item"><a class="nav-link active" data-toggle="tab" href="#data-penghargaan" role="tab" aria-selected="true"><span class="hidden-xs-down">Data Penghargaan</span></a></li>
                            </ul>
                            <div class="tab-content tabcontent-border col-12 p-0" style="margin-bottom: 2rem;">
                                <div class="tab-pane active row tab-scroll" id="data-penghargaan" role="tabpanel">
                                    <div class='col-md-12 nav-control' style="padding: 0px 5px;">
                                        <a style="font-size:18px;float: right;text-align: right;" class=""><span style="font-size:12.8px;padding: .5rem .5rem .5rem 1.25rem;margin: auto 0;" id="total-penghargaan" ></span></a>
                                    </div>
                                    <div class="col-md-12">
                                        <table class="table table-bordered table-condensed gridexample input-grid" id="input-penghargaan" data-table="Tab data penghargaan" style="width:100%;table-layout:fixed;word-wrap:break-word;white-space:nowrap">
                                        ...
                                        <a type="button" id="add-penghargaan" href="#" data-id="0" title="add-row" class="add-row btn btn-light2 btn-block btn-sm"><i class="saicon icon-tambah mr-1"></i>Tambah Baris</a>
                                    </div>
                                </div>
                            </div>
                        </div>
...
generateTable(
    "table-data",
    "{{ url('esaku-trans/sdm-adm-penghargaans') }}", 
    [
        {
            "targets": 0,
...
function addRowPenghargaanDefault() { 
    $('#input-penghargaan tbody').empty()
    var no= $('#input-penghargaan tbody > tr').length;
    no = no + 1;
    var idNama = 'nama-ke__'+no
    var idTanggal = 'penyelenggara-ke__'+no
    ...
    $('#input-penghargaan tbody').append(html)
...
function addRowPelatihan() { 
    ...
    $('#input-penghargaan tbody').append(html)
    $('#input-penghargaan tbody tr').not(':last').removeClass('selected-row');
...
$('#input-penghargaan tbody').on('click', '.hapus-item', function() {
    $(this).closest('tr').remove();
    no=1;
    $('#input-penghargaan tbody').children('.row-grid').each(function(){
...
$('#add-penghargaan').click(function() {
    var row = $('#input-penghargaan tbody tr').length
    if(row > 0) {
        var empty = false;
        var kolom = null;
        var baris = null;
        var error = '';
        $('#input-penghargaan tbody tr').each(function() {
            baris = $(this).index() + 1
            $(this).find('td').not(':first, :last, :eq(3)').each(function() {
                if($(this).text().trim() === '') {
                    empty = true;
                    kolom = $('#input-penghargaan thead > tr th').eq($(this).index()).text()
...
$('#input-penghargaan tbody').on('click', 'td', function(event) {
...
$('#input-penghargaan tbody').on('keydown', 'input', function(event) {
    event.stopPropagation();
...
$('#input-penghargaan tbody').on('change', 'input[type="file"]', function() {
    var td = $(this).parent()
    var id = $(td).attr('id')
...
function checkTablePenghargaan() {
    var nama = $('#input-penghargaan').attr('data-table')
    var table = $('#input-penghargaan tbody').children('tr')
    var kolom = ''
    var error = ''
    var baris = null
    var empty = false
    if(table.length === 0) {
        ...
        $('#input-penghargaan tbody').children('tr').each(function() {
            baris = $(this).index() + 1
            $(this).children('td').not(':first, :last, :eq(3)').each(function() {
                if($(this).text().trim() === '') {
                    empty = true
                    kolom = $('#input-penghargaan thead > tr th').eq($(this).index()).text()
...
$('#form-tambah').validate({
    ...
    submitHandler: function (form, event) {
        ...
        var id = $('#id').val();
        if(parameter == "true"){
            var url = "{{ url('esaku-trans/sdm-adm-penghargaan-update') }}";
            var pesan = "updated";
            var text = "Perubahan data "+id+" telah tersimpan";
        } else {
            var url = "{{ url('esaku-trans/sdm-adm-penghargaan') }}";
            var pesan = "saved";
...
function editData(id) { 
    ...
    $.ajax({
        type: 'GET',
        url: "{{ url('esaku-trans/sdm-adm-penghargaan') }}",
        data: { kode: id },
        dataType: 'json',
        async:false,
