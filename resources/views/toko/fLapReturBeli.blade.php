...
<div class="row" style="">
        <div style="z-index: 1;position: fixed;right: auto;left: auto;margin-right: 15px;margin-left: 25px;margin-top:15px" class="col-sm-12" id="subFixbar">
            <div class="card " id="sai-rpt-filter-box;" style="padding:10px;">
                <div class="card-body" style="padding: 0px;">
                    <h4 class="card-title pl-1" style="font-size:16px"><i class='fas fa-file'></i> Laporan Retur Pembelian</h4>
                    <hr>
                    <form id="formFilter">
        ...
        <div class="row" style="margin-left: -5px;">
            <div class="col-sm-12">
                <div class="form-group" style='margin-bottom:0'>
                    ...
                    <option value="" disabled selected>Pilih NIK Kasir</option>
                    </select>
                </div>
            </div>
        </div>
        <div class="row" style="margin-left: -5px;">
            <div class="col-sm-12">
                <div class="form-group" style='margin-bottom:0'>
                    ...
                    <option value="" disabled selected>Pilih No Bukti</option>
                    </select>
                </div>
            </div>
        </div>
    ...
    function getPeriode() {
        $.ajax({
            type:'GET',
            url:"{{url('toko-report/filter-periode-retur')}}",
            dataType: 'json',
            async: false,
            success: function(result) {
                if(result.status) {
                    for(var i=0;i<result.daftar.length;i++){
                        ...
                        $('#periode, #periode2').append(`<option value='${line.periode}'>${line.periode}</option>`)
                    }
    ...
    function getNik() {
        $.ajax({
            type:'GET',
            url:"{{url('toko-report/filter-nik-retur')}}",
            dataType: 'json',
            async: false,
            success: function(result) {
                if(result.status) {
                    for(var i=0;i<result.daftar.length;i++){
                        var line = result.daftar[i];
                        $('#nik_kasir').append(`<option value='${line.nik_user}'>${line.nama}</option>`)
                    }
                }else if(!result.status && result.daftar.message == "Unauthorized"){
                    Swal.fire({
                        title: 'Session telah habis',
    ...
    function getNoBukti() {
        $.ajax({
            type:'GET',
            url:"{{url('toko-report/filter-bukti-retur')}}",
            dataType: 'json',
            async: false,
            success: function(result) {
                if(result.status) {
                    for(var i=0;i<result.daftar.length;i++){
                        var line = result.daftar[i];
                        $('#no_bukti').append(`<option value='${line.no_bukti}'>${line.keterangan}</option>`)
                    }
                }else if(!result.status && result.daftar.message == "Unauthorized"){
                    Swal.fire({
                        title: 'Session telah habis',
    ...
    $('.card-body').on('submit', '#formFilter', function(e){
        ...
        xurl = "{{ url('/toko-auth/form')}}/rptReturBeli";
