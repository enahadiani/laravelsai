    ...
    getDataTypeAhead("{{ url('yakes-master/helper-neraca') }}","kode_neraca","kode_neraca");
    ...
    function getNeraca(id,target1,target2,target3,jenis){
        ...
        $.ajax({
            type: 'GET',
            url: "{{ url('/yakes-master/helper-neraca') }}/" + kode,
            dataType: 'json',
            async:false,
            success:function(result){
                if(result.status){
    ...
    function addRowGrid(param) {
        ...
        $(".search-neracake"+no).showFilter({
            title: 'Daftar Neraca',
            url: "{{ url('yakes-master/helper-neraca') }}",
            header:['Kode','Nama'],
            columns:[
                        { data: 'kode_neraca' },
                        { data: 'nama' }
    ...
    $('#input-grid').on('keydown','.inp-kode, .inp-nama,.inp-fs, .inp-nama_fs',function(e){
        var code = (e.keyCode ? e.keyCode : e.which);
        var nxt = ['.inp-kode','.inp-nama','.inp-fs','.inp-nama_fs'];
        var nxt2 = ['.td-kode','.td-nama','.td-fs','.td-nama_fs'];
        if (code == 13 || code == 9) {
            e.preventDefault();
    ...
    $('#form-tambah').validate({
        ...
        submitHandler: function (form) {
            ...
            // $iconLoad.show();
            if(param == "edit"){
                var url = "{{ url('/yakes-trans/setting-grafik') }}/"+$noBukti+"/"+$periode;
            }else{
                                ...
                                <table id='table-ju-preview' class='table table-bordered'>
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="width:8%; text-align:center;">Kode Neraca</th>
                                            <th style="width:15%; text-align:center;">Nama Neraca</th>
