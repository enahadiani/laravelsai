...
<meta charset="UTF-8">
    ...
    <style>
        ...
        .logo{
            background:url("{{ asset('img/ittpln.jpeg') }}") no-repeat;
    ...
    <!-- <script src="https://code.highcharts.com/modules/exporting.js"></script> -->
    <script src="{{ asset('dash-asset/dash-itpln/circle/circle-progress.js') }}"></script>
...
<nav class="navbar fixed-top px-0 py-0">
        ...
        <div class="navbar-right" >
            <div class="user d-inline-block mr-3 dropdown">
                <button class="btn btn-empty p-0" id="btn-admin" type="button" data-toggle="dropdown" aria-haspopup="true"
                    ...
                    @else
                    <img alt="Profile Picture" src="{{ config('api.url').'tarbak-auth/storage2/'.Session::get('foto') }}" style="width:40px;height:40px"/>
                    @endif
                    </span>
                </button>
                    ...
                    <a href="#" class="dropdown-profile">
                        <div style="height: 45px;padding: 0 1rem;">
                            <span id="adminProfilePhoto">
                                ...
                                @else
                                <img alt="Profile Picture" class="imgprofile ml-0" src="{{ config('api.url').'tarbak-auth/storage2/'.Session::get('foto') }}" style="width:40px;height:40px"/>
                                @endif
                            </span>
    ...
    function msgDialog(data){
        ...
        switch(data.type){
            ...
            case 'logout':
                ...
                //     window.location.href = "{{ url('dash-itpln/logout') }}";
                // }
        ...
        $.ajax({
            ...
            error: function(jqXHR, textStatus, errorThrown) {       
                if(jqXHR.status == 422){
                    ...
                    window.location="{{ url('/dash-itpln/sesi-habis') }}";
    ...
    function updateNotifRead(){
        $.ajax({
            type: 'POST',
            url: "{{ url('dash-itpln/notif-update-status') }}",
            dataType: 'json',
            async:false,
            success:function(result){    
                $('.count').remove();
    ...
    function loadForm(url){
        $.ajax({
            type: 'GET',
            url: "{{ url('dash-itpln/cek_session') }}",
            dataType: 'json',
            async:false,
            success:function(result){    
                if(!result.status){
    ...
    function getFormList() {
        $.ajax({
            type:'GET',
            url:"{{url('dash-itpln/search-form-list2')}}",
            dataType: 'json',
            async: false,
            success: function(result) {
                if(result.status) {
    ...
    function searchForm(cari){
        $.ajax({
            type: 'POST',
            url: "{{ url('dash-itpln/search-form') }}",
            dataType: 'json',
            data:{'cari':cari},
            async:false,
            success:function(result){  
                if(result.data.success.status){
                    if(result.data.success.data.length > 0){
                        ...
                        loadForm("{{ url('dash-itpln/form')}}/"+form);
                        return false;
                    }
                }else{
    ...
    function loadProfile(){
        loadForm("{{url('dash-itpln/form/fProfile')}}");
    }
    
    function loadMenu(){
        $.ajax({
            type: 'GET',
            url: "{{ url('dash-itpln/menu') }}",
            dataType: 'json',
            async:false,
            success:function(result){  
                if(result[0].status){
    ...
    $('.sub-menu').on('click','.a_link',function(e){
        ...
        var url = "{{ url('dash-itpln/form')}}/"+form;
        console.log(url);
        if(form == "" || form == "-"){
            // alert('Form dilock!');
            return false;
    ...
    //     source: function (cari, result) {
    //         $.ajax({
    //             url: "{{ url('dash-itpln/search-form-list') }}",
    //             data: {cari:cari},            
    //             dataType: "json",
    //             type: "GET",
    ...
    $('.to-home').click(function(){
        if(form != "" || form != "-"){
            ...
            loadForm("{{ url('dash-itpln/form') }}/blankform");
        }
    });
    ...
    if (mode == "dark") {
        ...
        var url_logo = "{{ asset('img/ittpln.jpeg') }}";
        var url_logo_mob = "{{ asset('img/ittpln.jpeg') }}";
        $('.logo').css({
