    ...
    <!-- LIST DATA -->
    <x-list-data judul="Data Video" tambah="true" :thead="array('ID','Tanggal','Keterangan','Aksi')" :thwidth="array(20,25,45,10)" :thclass="array('','','','text-center')" />
    <!-- END LIST DATA -->
                            ...
                            <div class="form-group col-md-6 col-sm-12">
                                <div class="row">
                                    <div class="col-md-12 col-sm-12">
                                        ...
                                        <textarea class="form-control" rows="2" id="keterangan" name="keterangan" required></textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="form-group col-md-6 col-sm-12">
                                <div class="row">
                                    <div class="col-md-12 col-sm-12">
                                        ...
                                        <input class="form-control" type="link" id="link" name="link" required>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    $('#saku-datatable').on('click', '#btn-tambah', function(){
        $('#row-id').hide();
        $('#id_edit').val('');
        $('#judul-form').html('Tambah Data Video');
    ...
    $('#form-tambah').validate({
        ...
        submitHandler: function (form) {
            ...
            if(parameter == "edit"){
                var url = "{{ url('telu-master/video') }}/"+id;
                var pesan = "updated";
                var text = "Perubahan data "+id+" telah tersimpan";
            }else{
                var url = "{{ url('telu-master/video') }}";
                var pesan = "saved";
    ...
    function hapusData(id){
        $.ajax({
            type: 'DELETE',
            url: "{{ url('telu-master/video') }}/"+id,
            dataType: 'json',
            async:false,
            success:function(result){
                if(result.data.status){
                    dataTable.ajax.reload();                    
                    showNotification("top", "center", "success",'Hapus Data','Data Video ('+id+') berhasil dihapus ');
                    $('#modal-pesan-id').html('');
    ...
    $('#saku-datatable').on('click', '#btn-edit', function(){
        ...
        $('#judul-form').html('Edit Data Video');
        editData(id);
    });
    // END BUTTON EDIT
    ...
    $('#table-data tbody').on('click','td',function(e){
        if($(this).index() != 3){
            ...
            $('#modal-preview-judul').css({'margin-top':'10px','padding':'0px !important'}).html('Preview Data Video').removeClass('py-2');
