    ...
    function getKlp(id,pp=null){
        var tmp = id.split(" - ");
        kode = tmp[0];
        var toUrl = "{{ url('dash-tarbak-trans/setting-rasio-klp') }}";
        $.ajax({
            type: 'GET',
            url: toUrl,
            dataType: 'json',
    ...
    getDataTypeAhead("{{ url('dash-tarbak-master/fs') }}","kode_fs","kode_fs");
    // getDataTypeAhead("{{ url('dash-tarbak-trans/setting-rasio-neraca') }}","kode_neraca","kode_neraca");
    ...
    function getNeraca(id,target1,target2,target3,jenis){
        ...
        $.ajax({
            type: 'GET',
            url: "{{ url('/dash-tarbak-trans/setting-rasio-neraca') }}/" + kode,
            dataType: 'json',
            async:false,
            success:function(result){
                if(result.status){
    ...
    function getFS(id,target1,target2,jenis){
        ...
        $.ajax({
            type: 'GET',
            url: "{{ url('/dash-tarbak-master/fs') }}/"+ kode,
            dataType: 'json',
            async:false,
            success:function(result){
                if(typeof result.daftar !== 'undefined' && result.daftar.length>0){
    ...
    var dataTable = generateTable(
        "table-data",
        "{{ url('dash-tarbak-trans/setting-rasio') }}", 
        [
            {
                "targets": [3],
                "visible": false,
        ...
        switch(id){
            ...
            case 'kode_klp' :
                var settings = {
                    id : id,
                    header : ['Kode', 'Nama'],
                    url : "{{ url('dash-tarbak-trans/setting-rasio-klp') }}",
                    columns : [
                        { data: 'kode_klp' },
                        { data: 'nama' }
                    ],
        ...
        switch(id){
            case 'kode_neraca[]' :
                ...
                var settings = {
                    id : id,
                    header : ['Kode', 'Nama'],
                    url : "{{ url('dash-tarbak-trans/setting-rasio-neraca') }}",
                    columns : [
                        { data: 'kode_neraca' },
                        { data: 'nama' }
                    ],
    ...
    $('#form-tambah').validate({
        ...
        submitHandler: function (form) {
            ...
            // $iconLoad.show();
            if(param == "edit"){
                var url = "{{ url('/dash-tarbak-trans/setting-rasio') }}";
            }else{
                var url = "{{ url('/dash-tarbak-trans/setting-rasio') }}";
            }
                $.ajax({
    ...
    $('#table-data tbody').on('click','td',function(e){
        if($(this).index() != 3){
            ...
            $.ajax({
                type: 'GET',
                url: "{{ url('/dash-tarbak-trans/setting-rasio-detail') }}",
                dataType: 'json',
                data:{kode_rasio: id},
                async:false,
                success:function(result){
    ...
    function hapusData(id){
        $.ajax({
            type: 'DELETE',
            url: "{{ url('dash-tarbak-trans/setting-rasio') }}",
            dataType: 'json',
            data:{kode_rasio:id},
            async:false,
            success:function(result){
