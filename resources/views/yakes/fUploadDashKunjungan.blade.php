    ...
    <x-list-data judul="Data Upload Kunjungan" tambah="true" :thead="array('No Bukti','Tanggal','Periode','Keterangan','Total Baris')" :thwidth="array(15,15,10,50,10)" :thclass="array('','','','','')" />
    <!-- END LIST DATA -->
            ...
            <div class="col-sm-12">
                <div class="card" style=''>
                    <div class="card-body form-header" style="padding-top:1rem;padding-bottom:1rem;">
                        <h6 id="judul-form" style="position:absolute;top:25px">Upload Dash Kunjungan</h6>
    ...
    $('#form-tambah').validate({
        ...
        submitHandler: function (form) {
            ...
            // $iconLoad.show();
            if(param == "edit"){
                var url = "{{ url('yakes-trans/dashKunjungan') }}";
            }else{
                var url = "{{ url('yakes-trans/dashKunjungan') }}";
            }
    ...
    $("#form-import").validate({
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                ...
                type: 'POST',
                url: "{{ url('yakes-trans/dashKunjungan-import') }}",
                dataType: 'json',
                data: formData,
                // async:false,
                contentType: false,
                ...
                success:function(result){
                    if(result.data.status){
                        if(result.data.validate){
                            ...
                            var link = "{{ config('api.url').'yakes-trans/dashKunjungan-export' }}?nik_user="+nik_user+"&periode="+periode+"&type=non";
    ...
    $('#process-upload').click(function(e){
        $.ajax({
            type: 'GET',
            url: "{{ url('yakes-trans/dashKunjungan-tmp') }}",
            dataType: 'json',
    ...
    $('#download-template').click(function(){
        var periode = $('#periode')[0].selectize.getValue();
        var nik_user = "{{ Session::get('nikUser') }}";
        var link = "{{ config('api.url').'yakes-trans/dashKunjungan-export' }}?nik_user="+nik_user+"&periode="+periode+"&type=template";
