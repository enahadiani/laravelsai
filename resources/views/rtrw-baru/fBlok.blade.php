    ...
    <!-- LIST DATA -->
    <x-list-data judul="Data Blok" tambah="true" :thead="array('Blok','Kode RT','Kode RW','Aksi')" :thwidth="array(20,50,20,10)" :thclass="array('','','','text-center')" />
    <!-- END LIST DATA -->
                    ...
                    <div class="card-body pt-3 form-body">
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6 col-sm-12">
                                ...
                                <input class="form-control" type="text" placeholder="Blok" id="blok" name="blok" required>                         
                            </div>
                        </div>
    ...
    $('#saku-datatable').on('click', '#btn-tambah', function(){
        $('#row-id').hide();
        $('#id_edit').val('');
        $('#judul-form').html('Tambah Data Blok');
    ...
    $('#form-tambah').validate({
        ...
        submitHandler: function (form, event) {
            ...
            var id = $('#blok').val();
            if(parameter == "edit"){
                var url = "{{ url('rtrw-master/blok') }}/"+id;
                var pesan = "updated";
                var text = "Perubahan data "+id+" telah tersimpan";
            }else{
                var url = "{{ url('rtrw-master/blok') }}";
                var pesan = "saved";
            ...
            $.ajax({
                ...
                success:function(result){
                    if(result.data.status){
                        ...
                        if(result.data.kode == "-" && result.data.jekode_kota != undefined){
                            msgDialog({
                                id: id,
                                type: result.data.jekode_kota,
                                text:'Kode Blok sudah digunakan'
                            });
                        }else{
    ...
    function hapusData(id){
        $.ajax({
            ...
            success:function(result){
                if(result.data.status){
                    dataTable.ajax.reload();                    
                    showNotification("top", "center", "success",'Hapus Data','Data Blok ('+id+') berhasil dihapus ');
                    $('#modal-pesan-id').html('');
    ...
    $('#saku-datatable').on('click', '#btn-edit', function(){
        ...
        $('#judul-form').html('Edit Data Blok');
        editData(id);
    });
    ...
    $('#kode_rw,#kode_pp,#blok').keydown(function(e){
        var code = (e.keyCode ? e.keyCode : e.which);
        var nxt = ['kode_rw','kode_pp','blok'];
        if (code == 13 || code == 40) {
            e.preventDefault();
