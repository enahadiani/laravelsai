    ...
    function getKlp(id,pp=null){
        var tmp = id.split(" - ");
        kode = tmp[0];
        var toUrl = "{{ url('telu-trans/setting-grafik-klp') }}";
        $.ajax({
            type: 'GET',
            url: toUrl,
            dataType: 'json',
    ...
    getDataTypeAhead("{{ url('telu-trans/setting-grafik-neraca') }}","kode_neraca","kode_neraca");
    ...
    function getNeraca(id,target1,target2,target3,jenis){
        ...
        $.ajax({
            type: 'GET',
            url: "{{ url('/telu-trans/setting-grafik-neraca') }}/" + kode,
            dataType: 'json',
            async:false,
            success:function(result){
                if(result.status){
    ...
    $(".div-kode_klp").showFilter({
        ...
        url: "{{ url('telu-trans/setting-grafik-klp') }}",
        header:['Kode','Nama'],
        columns:[
            { data: 'kode_klp' },
            { data: 'nama' }
    ...
    function addRowGrid(param) {
        ...
        $(".search-neracake"+no).showFilter({
            title: 'Daftar Neraca',
            url: "{{ url('telu-trans/setting-grafik-neraca') }}",
            header:['Kode','Nama'],
            columns:[
                        { data: 'kode_neraca' },
                        { data: 'nama' }
    ...
    $('#form-tambah').validate({
        ...
        submitHandler: function (form) {
            ...
            // $iconLoad.show();
            if(param == "edit"){
                var url = "{{ url('/telu-trans/setting-grafik') }}";
            }else{
                var url = "{{ url('/telu-trans/setting-grafik') }}";
            }
    ...
    $('#table-data tbody').on('click','td',function(e){
        if($(this).index() != 5){
            ...
            $.ajax({
                type: 'GET',
                url: "{{ url('/telu-trans/setting-grafik-detail') }}",
                dataType: 'json',
                data:{kode_grafik: id},
                async:false,
                success:function(result){
    ...
    function hapusData(id){
        $.ajax({
            type: 'DELETE',
            url: "{{ url('telu-trans/setting-grafik') }}",
            dataType: 'json',
            data:{kode_grafik:id},
            async:false,
            success:function(result){
