...
class AuthController extends Controller
{
    public function index()
    {
        ...
        if(!Session::get('login')){
            return redirect('midtrans/login')->with('alert','Kamu harus login dulu');
        }
        else{
            return view('midtrans.main');
        }
        ...
        try {
            ...
            if ($response->getStatusCode() == 200) { // 200 OK
                ...
                if($data["message"] == "success"){
                    ...
                    return redirect('midtrans/login')->with('alert','Password atau NIK, Salah !');
                }
            ...
            return redirect('midtrans/login')->with('alert',$res["message"]);
        }
        
    }

    public function login(){
        return view('midtrans.login');
    }

    public function logout(){
        Session::flush();
        return redirect('midtrans/login')->with('alert','Kamu sudah logout');
    }

    public function getMenu(){
        $client = new Client();
        $kodemenu = Session::get('kodeMenu');
        $response = $client->request('GET',  config('api.url').'gl//menu/'.$kodemenu,[
            'headers' => [
