                ...
                <div class="card-body pt-0 form-body" id="form-body">
                    <div class="form-row">
                        <div class="form-group col-md-6 col-sm-12">
                            <label for="kode_sdm">Kode Status Karyawan</label>
                            <input class="form-control" type="text" placeholder="Kode Status Karyawan" id="kode_sdm" name="kode_sdm" autocomplete="off" required>
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group col-md-12 col-sm-12">
                            <label for="nama">Nama</label>
                            <input class="form-control" type="text" placeholder="Nama Status Karyawan" id="nama" name="nama" autocomplete="off" required>
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group col-md-6 col-sm-12">
...
// TRIGGER FORM
    $('#saku-datatable').on('click', '#btn-tambah', function() {
        $('#judul-form').html('Tambah Data Status Karyawan');
        $('#kode_sdm').attr('readonly', false);
        newForm();
    });
...
// SAVE FORM TRIGGER
    $('#form-tambah').validate({
        ...
        submitHandler: function (form, event) {
            ...
            var id = $('#id').val();
            if(parameter == "true"){
                var url = "{{ url('esaku-master/sdm-status-update') }}";
                var pesan = "updated";
                var text = "Perubahan data "+id+" telah tersimpan";
            } else {
                var url = "{{ url('esaku-master/sdm-status') }}";
                var pesan = "saved";
            ...
            $.ajax({
                ...
                success:function(result){
                    if(result.data.status){
                        ...
                        last_add(dataTable,"kode_sdm", kode);
                        $('#id_edit').val('false')
...
// EDIT FORM TRIGGER
    ...
    function editData(id) { 
        ...
        $('#judul-form').html('Edit Data Status Karyawan');

        $.ajax({
            type: 'GET',
            url: "{{ url('esaku-master/sdm-status') }}",
            data: { kode: id },
            dataType: 'json',
            async:false,
