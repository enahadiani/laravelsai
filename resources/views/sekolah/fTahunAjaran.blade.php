  <link href="{{ asset('asset_elite/dist/css/custom.css') }}" rel="stylesheet">
    ...
    var dataTable = $('#table-data').DataTable({
        ...
        "order": [[0, "desc"]],
        'ajax': {
            'url': "{{ url('/sekolah/getTahunAjaran') }}",
            'async':false,
            'type': 'GET',
   ...
   function getPP(){
    ...
    e.preventDefault();
        ...
        var id = $('#kode_ta').val();
        if(parameter == "edit"){
            var url = "{{ url('/sekolah/postTahunAjaran') }}/"+id;
            var pesan = "updated";
        }else{
            var url = "{{ url('/sekolah/postTahunAjaran') }}";
            var pesan = "saved";
        }
        ...
        $.ajax({
            type: 'GET',
            url: "{{ url('sekolah/getTahunAjaran') }}/" + id + "/" + kode_pp,
            dataType: 'json',
            async:false,
        ...
        confirmButtonText: 'Yes, delete it!'
            ...
            if (result.value) {
                ...
                $.ajax({
                    type: 'DELETE',
                    url: "{{ url('sekolah/deleteTahunAjaran') }}/"+kode +"/"+ kode_pp,
                    dataType: 'json',
                    async:false,
                    success:function(result){
                        if(result.data.status){
