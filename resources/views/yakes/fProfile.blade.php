    ...
    <div class="col-12">
        <div class="row" >
            ...
            <div class="col-12 col-lg-7 col-xl-8 col-right">
                <div class="card mb-4">
                    <div class="card-body">
                        <h6 style="font-weight: bold;">Profile Pekerjaan</h6>
                        <table class="table table-profile">
                            <tbody>
                                <tr>
            ...
            if(result.status){
                switch(index){
                    ...
                    if(result.data[0].background == "-" || result.data[0].background == "" || result.data[0].background == undefined){
                        ...
                        var foto = "{{ config('api.url').'yakes-auth/storage' }}/"+result.data[0].background;
...
function initDash(){
    loadService('profile','GET',"yakes-auth/profile"); 
}
initDash();
...
$('#form-ubahPass').on('submit', function(e){
    e.preventDefault();
        ...
        var url = "yakes-auth/update-password";
        var pesan = "saved";
...
$('.crop_image').click(function(event){
    $image_crop.croppie('result', {
        ...
        if(tipe == 'foto'){
            var toUrl = "{{ url('yakes-auth/update-foto') }}";
        }else{
            var toUrl = "{{ url('yakes-auth/update-background') }}";
        }
            ...
            success:function(result)
            {
                if(result.data.status){
                    ...
                    if(tipe == 'foto'){
                        ...
                        var foto = "{{ config('api.url').'yakes-auth/storage' }}/"+result.data.foto;
