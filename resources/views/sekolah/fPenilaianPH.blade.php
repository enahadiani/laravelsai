    <style>
        ...
        #input-nilai td:not(:nth-child(1)):not(:nth-child(6)):hover
        {
            background:#f8f8f8;
            color:black;
        }
        ...
        <div class="col-12">
            <div class="card" >
                <div class="card-body pb-3" style="padding-top:1rem;">
                    <h6 style="position:absolute;top: 25px;">Data Penilaian Multi PH</h6>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document" style="max-width:800px">
            <div class="modal-content" style="border-radius:0.75em">
                <div class="modal-header py-0" style="display:block;height:49px">
                    <h6 class="modal-title py-2" style="position: absolute;">Preview Data Penilaian Multi PH <span id="modal-preview-nama"></span><span id="modal-preview-id" style="display:none"></span><span id="modal-preview-kode" style="display:none"></span> </h6>
    ...
    var dataTable = $("#table-data").DataTable({
        ...
        'ajax': {
            'url': "{{ url('sekolah-trans/penilaian-multiph') }}",
            'async':false,
            'type': 'GET',
    ...
    function showFilter(param,target1=null,target2=null){
        ...
        switch(par){
            ...
            case 'kode_kd': 
                header = ['Kode KD', 'Nama'];
                var toUrl = "{{ url('sekolah-trans/penilaian-multiph-kd') }}";
                var columns = [
                    { data: 'kode_kd' },
                    { data: 'nama' }
                ];
            ...
            if ( $(this).hasClass('selected') ) {
                ...
                if($target4 != ""){
                    if($target4 != ""){
                        ...
                        else if(par == "kode_jenis[]"){
                            $($target).closest('tr').find($target4).click();
                            setTimeout(function() {  $($target).parents("tr").find(".inp-pelaksanaan").focus(); }, 50);
                        }
                        else{
                            $($target).closest('tr').find($target4).click();
                        }
    ...
    function getJenisPenilaian(id,target1,target2,target3,jenis){
        var tmp = id.split(" - ");
        kode = tmp[0];
        $.ajax({
            type: 'GET',
            url: "{{ url('sekolah-master/jenis-penilaian') }}",
            data:{kode_pp:$('#kode_pp').val(),kode_jenis:kode,flag_aktif:1},
            dataType: 'json',
            async:false,
            success:function(result){    
                if(result.status){
                    if(typeof result.daftar !== 'undefined' && result.daftar.length>0){
                        if(jenis == 'change'){
                            ...
                            $('.'+target3).closest('td').addClass("px-0 py-0 aktif");

                            $('.'+target1).closest('tr').find('.search-kode_jenis').hide();
                            $('.'+target1).val(id);
                            $('.td'+target1).text(id);
                            $('.'+target1).hide();
                            $('.td'+target1).show();
    ...
    $('#saku-datatable').on('click', '#btn-edit', function(){
        ...
        $.ajax({
            type: 'GET',
            url: "{{ url('sekolah-trans/penilaian-multiph-detail') }}",
            dataType: 'json',
    ...
    $('#form-tambah').validate({
        ...
        submitHandler: function (form) {
            ...
            // $iconLoad.show();
            if(param == "edit"){
                var url = "{{ url('sekolah-trans/penilaian-multiph') }}";
            }else{
                var url = "{{ url('sekolah-trans/penilaian-multiph') }}";
            }
    ...
    // ENTER FIELD FORM
    $('#kode_kelas,#kode_matpel,#kode_sem,#kode_kd').keydown(function(e){
        var code = (e.keyCode ? e.keyCode : e.which);
        var nxt = ['kode_kelas','kode_matpel','kode_sem','kode_kd'];
        if (code == 13 || code == 40) {
            e.preventDefault();
    ...
    function hideUnselectedRow() {
        $('#input-nilai > tbody > tr').each(function(index, row) {
            if(!$(row).hasClass('selected-row')) {
                ...
                var pelaksanaan = $('#input-nilai > tbody > tr:eq('+index+') > td').find(".inp-pelaksanaan").val();
                ...
                $('#input-nilai > tbody > tr:eq('+index+') > td').find(".td-kode_jenis").show();
                $('#input-nilai > tbody > tr:eq('+index+') > td').find(".inp-pelaksanaan").hide();
                $('#input-nilai > tbody > tr:eq('+index+') > td').find(".td-pelaksanaan").show();
            ...
            switch (idx) {
                ...
                case 3:
                    if(isi != ""){
                        ...
                        var noidx = $(this).parents("tr").find(".no-nilai").text();
                        var kode = $(this).val();
                        var target1 = "kode_jeniske"+noidx;
                        var target2 = "";
                        var target3 = "pelaksanaanke"+noidx;
                        getJenisPenilaian(kode,target1,target2,target3,'tab');     
                    }else{
                        alert('Kode Jenis yang dimasukkan tidak valid');
                        return false;
                    }
                    break;
                case 4:
        ...
        if(idx == 0){
            ...
            if($(this).hasClass('px-0 py-0 aktif')){
                ...
                if(idx == 4){
                    ...
                    $(this).parents("tr").find(".inp-kode_jenis").focus();
                }else{
                    $(this).parents("tr").find(".inp-kode_jenis").hide();
                    $(this).parents("tr").find(".td-kode_jenis").show();
                    $(this).parents("tr").find(".search-kode_jenis").hide();
                ...
                if(idx == 5){
                    ...
                    $(this).parents("tr").find(".inp-pelaksanaan").focus();
                }else{
                    $(this).parents("tr").find(".inp-pelaksanaan").hide();
                    $(this).parents("tr").find(".td-pelaksanaan").show();
                    $(this).parents("tr").find(".search-pelaksanaan").hide();
    ...
    $("#form-import").validate({
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                ...
                type: 'POST',
                url: "{{ url('sekolah-trans/import-multiph-excel') }}",
                dataType: 'json',
                data: formData,
                // async:false,
                contentType: false,
    ...
    $('#process-upload').click(function(e){
        $.ajax({
            type: 'GET',
            url: "{{ url('sekolah-trans/nilai-multiph-tmp') }}",
            dataType: 'json',
