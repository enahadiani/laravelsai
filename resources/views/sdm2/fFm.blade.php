...
{{-- SAKU TABLE --}}
<x-list-data judul="Data FM" tambah="true" :thead="array('Kode','Nama','Area','Aksi')" :thwidth="array(20,25,25,10)"
...
// TRIGGER FORM
    $('#saku-datatable').on('click', '#btn-tambah', function() {
        $('#judul-form').html('Tambah Data FM');
    ...
    $('#form-tambah').validate({
        ...
        submitHandler: function (form, event) {
            ...
            if(parameter == "true"){
                ...
                var url = "{{ url('esaku-master/sdm-fm-update') }}";
                var pesan = "updated";
                var text = "Perubahan data "+id+" telah tersimpan";
            } else {

                var url = "{{ url('esaku-master/sdm-fm') }}";
                var pesan = "saved";
...
// EDIT FORM TRIGGER
    ...
    function editData(id) {
        ...
        $('#judul-form').html('Edit Data FM');

        $.ajax({
            type: 'GET',
            url: "{{ url('esaku-master/sdm-fm') }}",
            data: { kode: id },
            dataType: 'json',
            async:false,
    ...
    function hapusData(id) {
        $.ajax({
            ...
            success:function(result){
                if(result.data.status){
                    dataTable.ajax.reload();
                    showNotification("top", "center", "success",'Hapus Data','Data FM ('+id+') berhasil dihapus ');
                    $('#modal-pesan-id').html('');
