...
<head>
    ...
    <style>
        ...
        .logo{
            background:url("{{ asset('img/ypt.jpeg') }}") no-repeat;
    ...
    <script src="{{ asset('asset_elite/sai.js?version=_').time() }}') }}"></script>
    <script src="{{ asset('asset_elite/inputmask.js') }}"></script>
    <script src="{{ asset('asset_dore/js/vendor/bootstrap-tagsinput.min.js') }}"></script>
    <script src="{{ asset('asset_dore/main.js?version=_').time() }}"></script>
...
@endif
                    ...
                    <a href="#" class="dropdown-profile">
                        <div style="height: 45px;padding: 0 1rem;">
                            <span id="adminProfilePhoto">
                                ...
                                @else
                                <img alt="Profile Picture" class="imgprofile ml-0" src="{{ config('api.url').'sukka-auth/storage/'.Session::get('foto') }}" style="width:40px;height:40px"/>
                                @endif
                            </span>
    ...
    function msgDialog(data){
        switch(data.type){
            ...
            case 'logout':
                ...
                //     window.location.href = "{{ url('sukka-auth/logout') }}";
                // }
        ...
        swalWithBootstrapButtons.fire({
            ...
            switch(data.type){
                ...
                case 'logout':
                    if (result.value) {
                        window.localStorage.setItem('logged_in', false);
                        window.localStorage.removeItem('sukka-form');
                        window.location.href = "{{ url('sukka-auth/logout') }}";
                    } else if (result.dismiss === Swal.DismissReason.cancel) {
                        // console.log('cancel');
                    }                    
    ...
    // if(window.localStorage.getItem('sukka-form') != "" && window.localStorage.getItem('sukka-form') != null && window.localStorage.getItem('sukka-form') != "-"){
    //     var form = window.localStorage.getItem('sukka-form');
    // }else{
    ...
    function updateNotifRead(){
        $.ajax({
            type: 'POST',
            url: "{{ url('sukka-auth/notif-update-status') }}",
            dataType: 'json',
            async:false,
            success:function(result){    
                $('.count').remove();
    ...
    var channel = pusher.subscribe('saisukka-channel-'+userNIK);
    channel.bind('saisukka-event', function(data) {
        getNotif();
    ...
    function loadForm(url){
        $.ajax({
            type: 'GET',
            url: "{{ url('sukka-auth/cek_session') }}",
            dataType: 'json',
            async:false,
            success:function(result){    
                if(!result.status){
    ...
    function getFormList() {
        $.ajax({
            type:'GET',
            url:"{{url('sukka-auth/search-form-list2')}}",
            dataType: 'json',
            async: false,
            success: function(result) {
                if(result.status) {
    ...
    function searchForm(cari){
        $.ajax({
            type: 'POST',
            url: "{{ url('sukka-auth/search-form') }}",
            dataType: 'json',
            data:{'cari':cari},
            async:false,
            success:function(result){  
    ...
    function loadProfile(){
        loadForm("{{url('sukka-auth/form/fProfile')}}");
    }
    
    function loadMenu(){
        $.ajax({
            type: 'GET',
            url: "{{ url('sukka-auth/menu') }}",
            dataType: 'json',
            async:false,
            success:function(result){  
                if(result[0].status){
    ...
    $('.sub-menu').on('click','.a_link',function(e){
        ...
        window.localStorage.setItem('sukka-form',form);
        $form_aktif = $(this).data('kode_form');
        $('.sub-menu li').removeClass('active');
        $(this).closest('li').addClass('active');
        var url = "{{ url('sukka-auth/form')}}/"+form;
        if(form == "" || form == "-"){
            // alert('Form dilock!');
            return false;
        }else{
    ...
    //     source: function (cari, result) {
    //         $.ajax({
    //             url: "{{ url('sukka-auth/search-form-list') }}",
    //             data: {cari:cari},            
    //             dataType: "json",
    //             type: "GET",
    ...
    $('.to-home').click(function(){
        ...
        if(home != "" || home != "-"){
            loadForm("{{ url('sukka-auth/form') }}/"+home);
        }else{
            loadForm("{{ url('sukka-auth/form') }}/blankform");
        }
    });
    ...
    <script>
        ...
        //     console.log('ok');
        //     loadForm("{{ url('sukka-auth/form') }}/fMasakun");
