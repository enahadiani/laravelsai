    ...
    <!-- LIST DATA -->
    <x-list-data judul="Data Pemasukan" tambah="true" :thead="array('No Kas Bank','Tanggal','Deskripsi','Nilai','Tgl Input','Aksi')" :thwidth="array(15,15,45,15,0,10)" :thclass="array('','','','','','text-center')" />
    <!-- END LIST DATA -->
    ...
    $('#saku-datatable').on('click', '#btn-tambah', function(){
        ...
        $('#id_edit').val('');
        $('.no_bukti').hide();
        $('#judul-form').html('Tambah Data Pemasukan');
        ...
        switch(id){
            ...
            case 'kode_ref': 
                options = {
                    id : id,
                    header : ['Kode', 'Nama'],
                    url : "{{ url('esaku-master/reftrans-pemasukan') }}",
                    columns : [
                        { data: 'kode_ref' },
                        { data: 'nama' }
                    ],
    ...
    $('#form-tambah').validate({
        ...
        submitHandler: function (form) {
            ...
            var id = $('#no_bukti').val();
            if(parameter == "edit"){
                var url = "{{ url('esaku-trans/pemasukan') }}/"+id;
                var pesan = "updated";
                var text = "Perubahan data "+id+" telah tersimpan";
            }else{
                var url = "{{ url('esaku-trans/pemasukan') }}";
                var pesan = "saved";
    ...
    function hapusData(id){
        $.ajax({
            type: 'DELETE',
            url: "{{ url('esaku-trans/pemasukan') }}/"+id,
            dataType: 'json',
            async:false,
            success:function(result){
                if(result.data.status){
                    dataTable.ajax.reload();                    
                    showNotification("top", "center", "success",'Hapus Data','Data Pemasukan ('+id+') berhasil dihapus ');
                    $('#modal-pesan-id').html('');
    ...
    $('#saku-datatable').on('click', '#btn-edit', function(){
        ...
        $('#judul-form').html('Edit Data Pemasukan');
        editData(id);
    });
    // END BUTTON EDIT
    ...
    $('#table-data tbody').on('click','td',function(e){
        if($(this).index() != 4){
            ...
            $('#modal-preview-judul').css({'margin-top':'10px','padding':'0px !important'}).html('Preview Data Pemasukan').removeClass('py-2');
