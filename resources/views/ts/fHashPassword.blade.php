                            ...
                            <table class="table table-bordered table-striped" id="table-data" >
                                <thead>
                                    <tr>
                                        ...
                                        <th style="width:20%">Kode PP</th>
                                        <th style="width:50%">Nama PP</th>
                                        <th style="width:20%">Jumlah NIK</th>
...
var tablepphash = $("#table-data").DataTable({
    ...
    $('#table-data').on('click','.btn-hash',function(e){
        ...
        $.ajax({
            ...
            url: "{{ url('ts-trans/hash-pass-pp') }}",
            dataType: 'json',
            data:{ kode_pp: kode_pp, kode_lokasi: kode_lokasi},
            async:false,
            success:function(result){
                if(result.status){
                    if($sts_all != 1){
        ...
        switch(id){
            case 'kode_lokasi' :
                var settings = {
                    id : id,
                    header : ['Kode', 'Nama'],
                    url : "{{ url('ts-trans/hash-pass-pp-lokasi') }}",
                    columns : [
                        { data: 'kode_lokasi' },
                        { data: 'nama' }
                    ],
