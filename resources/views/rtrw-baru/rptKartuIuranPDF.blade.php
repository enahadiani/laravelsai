...
<body>
    <div style='border-bottom: double #d7d7d7;padding:0 3rem'>
        <table class="table-borderless mb-2 table-kop-prev" width="100%" >
            <tr>
                <td width="25%" colspan="5" class="vtop"><h4 class="text-primary bold">KARTU IURAN</h4></td>
                <td width="75%" colspan="3" class="vtop text-right"><h4 class="mb-2 bold">{{ $lokasi[0]['nama'] }}</h4><p class="lh1">{{ $lokasi[0]['alamat'] }}<br>{{ $lokasi[0]['kota'] }} {{$lokasi[0]['kodepos'] }} </p><p class="mt-2">{{ $lokasi[0]['email'] }} | {{ $lokasi[0]['no_telp'] }}</p></td>
            </tr>
        </table>
    </div>
    @for($a=0; $a < count($data); $a++)
        ...
        <div style="padding:0 3.1rem">
            <table class="table table-striped table-body-prev mt-2" width="100%">
                <tr style="background-color:#0058E4!important;color:black !important">
