<link rel="stylesheet" href="{{ asset('master.css?version=_').time() }}" />
                    ...
                    <div class="card-body pt-3 form-body">
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6 col-sm-12">
                                ...
                                <div class="row">
                                    <div class="col-md-6 col-sm-12">
                                        <label for="kode_jab">Kode Jabatan (Approval)</label>
                                        <div class="input-group">
                                            <div class="input-group-prepend hidden" style="border: 1px solid #d7d7d7;">
                                                <span class="input-group-text info-code_kode_jab" readonly="readonly" title="" data-toggle="tooltip" data-placement="top" ></span>
                                            </div>
                                            <input type="text" class="form-control inp-label-kode_jab" id="kode_jab" name="kode_jab" value="" title="">
    ...
    function getJabatan(id=null){
        $.ajax({
            ...
            success:function(result){    
                if(result.status){
                    if(typeof result.daftar !== 'undefined' && result.daftar.length>0){
                        showInfoField('kode_jab',result.daftar[0].kode_jab,result.daftar[0].nama);
                    }else{
                        $('#kode_jab').attr('readonly',false);
                        $('#kode_jab').css('border-left','1px solid #d7d7d7');
                        $('#kode_jab').val('');
                        $('#kode_jab').focus();
                    }
                }
    ...
    $('#form-tambah').on('change', '#kode_jab', function(){
        var par = $(this).val();
        getJabatan(par);
    });
    ...
    $('#saku-datatable').on('click', '#btn-tambah', function(){
        ...
        $('#saku-datatable').hide();
        $('#saku-form').show();
        $('#kode_lokasi').parent('div.input-group').removeClass('readonly')
        ...
        $.ajax({
            ...
            success:function(res){
                ...
                if(result.status){
                    ...
                    $('#kode_lokasi').parent('div.input-group').addClass('readonly')
