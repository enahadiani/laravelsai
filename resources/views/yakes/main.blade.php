...
<head>
    ...
    <style>
        ...
        h1, h2, h3, h4, h5, h6, .h1, .h2, .h3, .h4, .h5, .h6, p,li,ul,a,input,select,.bootstrap-tagsinput,label{
            font-family: 'Roboto', sans-serif !important;
            color:black;
        }
...
@endif
        ...
        <div class="navbar-right" >
            <div class="user d-inline-block mr-3 dropdown">
                <button class="btn btn-empty p-0" id="btn-admin" type="button" data-toggle="dropdown" aria-haspopup="true"
                    ...
                    @else
                    <img alt="Profile Picture" src="{{ config('api.url').'yakes-auth/storage/'.Session::get('foto') }}" style="width:40px;height:40px"/>
                    @endif
                    </span>
                </button>
                    ...
                    <a href="#" class="dropdown-profile">
                        <div style="height: 45px;padding: 0 1rem;">
                            <span id="adminProfilePhoto">
                                ...
                                @else
                                <img alt="Profile Picture" class="imgprofile ml-0" src="{{ config('api.url').'yakes-auth/storage/'.Session::get('foto') }}" style="width:40px;height:40px" />
                                @endif
                            </span>
    ...
    function updateNotifRead(){
        $.ajax({
            type: 'POST',
            url: "{{ url('yakes-auth/notif-update-status') }}",
            dataType: 'json',
            async:false,
            success:function(result){    
                $('.count').remove();
    ...
    function loadForm(url){
        $.ajax({
            type: 'GET',
            url: "{{ url('yakes-auth/cek_session') }}",
            dataType: 'json',
            async:false,
            success:function(result){    
                if(!result.status){
    ...
    function getFormList() {
        $.ajax({
            type:'GET',
            url:"{{url('yakes-auth/search-form-list2')}}",
            dataType: 'json',
            async: false,
            success: function(result) {
                if(result.status) {
    ...
    function searchForm(cari){
        $.ajax({
            type: 'POST',
            url: "{{ url('yakes-auth/search-form') }}",
            dataType: 'json',
            data:{'cari':cari},
            async:false,
            success:function(result){  
    ...
    function loadProfile(){
        loadForm("{{url('yakes-auth/form/fProfile')}}");
    }
    
    function loadMenu(){
        $.ajax({
            type: 'GET',
            url: "{{ url('yakes-auth/menu') }}",
            dataType: 'json',
            async:false,
            success:function(result){  
                if(result[0].status){
    ...
    $('.sub-menu').on('click','.a_link',function(e){
        ...
        var url = "{{ url('yakes-auth/form')}}/"+form;
        console.log(url);
        if(form == "" || form == "-"){
            // alert('Form dilock!');
            return false;
    ...
    //     source: function (cari, result) {
    //         $.ajax({
    //             url: "{{ url('yakes-auth/search-form-list') }}",
    //             data: {cari:cari},            
    //             dataType: "json",
    //             type: "GET",
