...
<head>
    ...
    <style>
        ...
        .logo-mobile{
            background:url("{{ asset('img/logo-ypt2.jpg') }}") no-repeat;
...
@endif
        ...
        <div class="navbar-right" >
            <div class="user d-inline-block mr-3 dropdown">
                <button class="btn btn-empty p-0" id="btn-admin" type="button" data-toggle="dropdown" aria-haspopup="true"
                    ...
                    @else
                    <img alt="Profile Picture" src="{{ config('api.url').'ts/storage/'.Session::get('foto') }}" style="width:40px;height:40px"/>
                    @endif
                    </span>
                </button>
                    ...
                    <a href="#" class="dropdown-profile">
                        <div style="height: 45px;padding: 0 1rem;">
                            <span id="adminProfilePhoto">
                                ...
                                @else
                                <img alt="Profile Picture" class="imgprofile ml-0" src="{{ config('api.url').'ts/storage/'.Session::get('foto') }}" style="width:40px;height:40px" />
                                @endif
                            </span>
    ...
    <div class='navbar_bottom' style=''>
        ...
        <i class='saicon icon-rp' style='text-align: center;'></i><p style='text-align: center;font-size: 14px;margin-bottom:0'>Beranda</p></a>
        <a href='#notif' style='padding:10px 0px 0px 0px;width: 33%;'>
        <i class='saicon icon-notif' style='text-align: center;'></i><p style='text-align: center;font-size: 14px;margin-bottom:0'>Notifikasi</p></a>
    ...
    function msgDialog(data){
        switch(data.type){
            ...
            case 'logout':
                ...
                //     window.location.href = "{{ url('mobile-ts/logout') }}";
                // }
    ...
    function updateNotifRead(){
        $.ajax({
            type: 'POST',
            url: "{{ url('mobile-ts/notif-update-status') }}",
            dataType: 'json',
            async:false,
            success:function(result){    
                $('.count').remove();
    ...
    function loadForm(url){
        $.ajax({
            type: 'GET',
            url: "{{ url('mobile-ts/cek_session') }}",
            dataType: 'json',
            async:false,
            success:function(result){    
                if(!result.status){
                    window.location.href = "{{ url('mobile-ts/sesi-habis') }}";
                }else{
    ...
    function getFormList() {
        $.ajax({
            type:'GET',
            url:"{{url('mobile-ts/search-form-list2')}}",
            dataType: 'json',
            async: false,
            success: function(result) {
                if(result.status) {
            ...
            success:function(result){  
                if(result.data.success.status){
                    if(result.data.success.data.length > 0){
                        ...
                        loadForm("{{ url('mobile-ts/form')}}/"+form);
                        return false;
                    }
                }else{
    ...
    function loadProfile(){
        loadForm("{{url('mobile-ts/form/fProfile')}}");
    }
    
    function loadMenu(){
        $.ajax({
            type: 'GET',
            url: "{{ url('mobile-ts/menu') }}",
            dataType: 'json',
            async:false,
            success:function(result){  
                if(result[0].status){
    ...
    $('.sub-menu').on('click','.a_link',function(e){
        ...
        var url = "{{ url('mobile-ts/form')}}/"+form;
        console.log(url);
        if(form == "" || form == "-"){
            // alert('Form dilock!');
            return false;
    ...
    //     source: function (cari, result) {
    //         $.ajax({
    //             url: "{{ url('mobile-ts/search-form-list') }}",
    //             data: {cari:cari},            
    //             dataType: "json",
    //             type: "GET",
    ...
    $('.to-home').click(function(){
        if(form != "" || form != "-"){
            ...
            loadForm("{{ url('mobile-ts/form') }}/blankform");
        }
    });
