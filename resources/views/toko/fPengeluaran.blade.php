...
<div class="container-fluid mt-3" style="font-size:13px">
        <div class="row" id="saku-datatable">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title mb-4" style="font-size:16px"><i class='fas fa-cube'></i> Data Pengeluaran 
                ...
                <div class="card">
                    <form id="form-tambah" style=''>
                        <div class="card-body pb-0">
                            <h4 class="card-title mb-4" style="font-size:16px"><i class='fas fa-cube'></i> Form Data Pengeluaran
    ...
    var dataTable = $('#table-data').DataTable({
        ...
        // "scrollX": true,
        'ajax': {
            'url': "{{ url('toko-trans/pengeluaran') }}",
            'async':false,
            'type': 'GET',
    ...
    function showFilter(param,target1,target2){
        ...
        case 'kode_ref': 
            header = ['Kode', 'Nama'];
            var toUrl = "{{ url('toko-master/reftrans-pengeluaran') }}";
                var columns = [
                    { data: 'kode_ref' },
                    { data: 'nama' }
                ];
    ...
    $('#saku-form').on('submit', '#form-tambah', function(e){
        ...
        var id = $('#id').val();
        if(parameter == "edit"){
            var url = "{{ url('toko-trans/pengeluaran') }}/"+id;
            var pesan = "updated";
        }else{
            var url = "{{ url('toko-trans/pengeluaran') }}";
            var pesan = "saved";
        }
    ...
    $('#saku-datatable').on('click','#btn-delete',function(e){
        ...
        confirmButtonText: 'Yes, delete it!'
            ...
            if (result.value) {
                ...
                $.ajax({
                    type: 'DELETE',
                    url: "{{ url('toko-trans/pengeluaran') }}/"+id,
                    dataType: 'json',
                    async:false,
                    success:function(result){
                        if(result.data.status){
