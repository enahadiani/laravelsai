                ...
                <div class="card-body pt-3 form-body">
                        ...
                        <div class="form-group col-md-6 col-sm-12">
                            <div class="row">
                                ...
                                <div class="col-md-6 col-sm-12">
                                    ...
                                    <input class='form-control datepicker' id="waktu" name="waktu" autocomplete="off"
                                        value="{{ date('d/m/Y') }}">
                                    <i style="font-size: 18px;margin-top:30px;margin-left:5px;position: absolute;top: 0;right: 25px;"
                                        class="simple-icon-calendar date-search"></i>
                                </div>
                            ...
                            <div class="row">
                                <div class="col-md-6 col-sm-12">
                                    ...
                                    <input class="form-control" type="text" placeholder="Dasar/Latar Belakang"
                                        id="dasar" name="dasar" autocomplete="off" required>
                                </div>
                                <div class="col-md-6 col-sm-12">
                                ...
                                <div class="col-md-6 col-sm-12">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" class="form-control inp-label-nik_ver" id="nik_ver"
                                            name="nik_ver" autocomplete="off" data-input="cbbl" value="" title=""
                                            required readonly>
                    ...
                    <ul class="nav nav-tabs col-12 " role="tablist">
                        <li class="nav-item"><a class="nav-link active" data-toggle="tab" href="#data-barang" role="tab"
                                aria-selected="true"><span class="hidden-xs-down">Data Barang</span></a>
                        </li>
                        <li class="nav-item"><a class="nav-link" data-toggle="tab" href="#data-dokumen-po" role="tab"
                                aria-selected="false"><span class="hidden-xs-down">Data PO</span></a>
                        </li>
...
<div class="row" id="saku-history" style="display: none;">
    <div class="col-12">
        <div class="card" style="height: 100%;">
            <div class="card-body form-header" style="padding-top:1rem;padding-bottom:1rem;min-height:62.8px">
                <button type="button" class="btn btn-secondary ml-2" id="btn-aback" style="float:right;"><i
                        class="fa fa-undo"></i> Kembali</button>
            </div>
            <div class="separator mb-2"></div>
            ...
            success: function (res) {
                ...
                if (result.status) {
                    for (i = 0; i < result.daftar.length; i++) {
                        $dtKlpBarang[i] = {
                            id: result.daftar[i].kode_barang,
                            name: result.daftar[i].nama
                        };
                    }
            ...
            success: function (result) {
                if (result.status) {
                    ...
                    if (typeof result.daftar !== 'undefined' && result.daftar.length > 0) {
                        for (i = 0; i < result.daftar.length; i++) {
                            control.addOption([{
                                text: result.daftar[i].kode_pp + ' - ' + result.daftar[
                                    i].nama,
                                value: result.daftar[i].nama
                            }]);
                        }
                    }
                }
            ...
            success: function (result) {
                if (result.status) {
                    ...
                    if (typeof result.daftar !== 'undefined' && result.daftar.length > 0) {
                        for (i = 0; i < result.daftar.length; i++) {
                            control.addOption([{
                                text: result.daftar[i].no_bukti + ' - ' + result.daftar[
                                    i].no_bukti,
                                value: result.daftar[i].no_bukti
                            }]);
                        }
                    }
                }
            ...
            $(table).children('tbody').each(function (index, tbody) {
                $(tbody).children('tr').each(function (index, tr) {
                    $(tr).children('td').not(':first, :last').each(function (index, td) {
                        var value = $(td).children('input').not(
                            "input[type='hidden'], input[type='file']").val()
                        $(td).children('input').not(
                            "input[type='hidden'], input[type='file']").val(
                            value)
                        $(td).children('span').not('.not-show').text(value)
                        $(td).children('input').not(
                            "input[type='hidden'], input[type='file']").hide()
                        $(td).children('a').not('.hapus-item, .download-item')
                            .hide()
        ...
        if (row > 0) {
            ...
            $('#input-dokumen-po tbody tr').each(function () {
                ...
                $(this).find('td').not(':first, :last, :eq(2)').each(function () {
                    if ($(this).text().trim() === '') {
                        empty = true;
                        kolom = $('#input-dokumen-po thead > tr th').eq($(this).index())
                            .text()
                        error =
                            `Data pada kolom ${kolom} di baris nomor ${baris} tidak boleh kosong`
                        return false;
        ...
        if (row > 0) {
            ...
            $('#input-dokumen-compare tbody tr').each(function () {
                ...
                $(this).find('td').not(':first, :last, :eq(2)').each(function () {
                    if ($(this).text().trim() === '') {
                        empty = true;
                        kolom = $('#input-dokumen-compare thead > tr th').eq($(this)
                            .index()).text()
                        error =
                            `Data pada kolom ${kolom} di baris nomor ${baris} tidak boleh kosong`
                        return false;
            ...
            $('#input-barang tbody').children('tr').each(function () {
                ...
                $(this).children('td').not(':first, :last').each(function () {
                    if ($(this).text().trim() === '') {
                        ...
                        kolom = $('#input-barang thead > tr th').eq($(this).index()).text()
                        error =
                            `Data pada kolom ${kolom} di baris nomor ${baris} tidak boleh kosong di ${nama}`
                        return false;
                    }
                })
                if (empty) {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        if (result.data.status) {
                            ...
                            Swal.fire(
                                'Great Job!',
                                'Your data has been ' + pesan + ' ' + result.data
                                .message,
                                'success'
                            )
                            ...
                            if (result.data.kode == "-" && result.data.jenis != undefined) {
                                ...
                                Swal.fire({
                                    ...
                                    footer: '<a href>' + result.data.message +
                                        '</a>'
                                })
                            }
                        }
