            ...
            if(result.status){
                switch(index){
                    ...
                    if(result.data[0].foto == "-" || result.data[0].foto == "" || result.data[0].foto == undefined){
                        ...
                        <img alt="Profile" src="https://api.simkug.com/api/ypt-auth/storage2/`+result.data[0].foto+`" class="img-thumbnail card-img social-profile-img" style="border-radius: 50%;width:120px;height:120px;">
                        `;
                    }
                    ...
                    if(result.data[0].background == "-" || result.data[0].background == "" || result.data[0].background == undefined){
                        ...
                        var background = `<img class="social-header card-img" style="height:200px;object-position:bottom" src="https://api.simkug.com/api/ypt-auth/storage2/`+result.data[0].background+`" />`;
                    }
...
function initDash(){
    loadService('profile','GET',"dash-itpln/profile"); 
}
initDash();
...
$('#form-ubahPass').on('submit', function(e){
    e.preventDefault();
        ...
        var url = "dash-itpln/update-password";
        var pesan = "saved";
...
$('.crop_image').click(function(event){
    $image_crop.croppie('result', {
        ...
        if(tipe == 'foto'){
            var toUrl = "{{ url('dash-itpln/update-foto') }}";
        }else{
            var toUrl = "{{ url('dash-itpln/update-background') }}";
        }
            ...
            success:function(result)
            {
                if(result.data.status){
                    ...
                    if(tipe == 'foto'){
                        ...
                        var foto = "{{ config('api.url').'ypt-auth/storage2' }}/"+result.data.foto;
