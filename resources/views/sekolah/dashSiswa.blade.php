    ...
    <style>
        ...
        {
            height:180px;
            background-image:url("{{ asset('asset_elite/images/user.png') }}");
    ...
    <style>
        #profile-photo
        {
            ...
            height:180px;
            background-image:url("{{ config('api.url').'sekolah/storage/'.Session::get('foto') }}"); background-position: center; /* Center the image */
            background-repeat: no-repeat; /* Do not repeat the image */
            background-size: cover;
            border-radius:0.75rem; 
        }
    ...
    function getProfile(){
        $.ajax({
            type: "GET",
            url: "{{ url('sekolah-dash/dash-siswa-profile') }}",
            dataType: 'json',
            data: {},
