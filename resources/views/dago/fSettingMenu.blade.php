...
<div class="modal fade" id="sai-treegrid-modal">
    ...
    function init(kode_klp=null){
        ...
        $.ajax({
            type: 'GET',
            url: "{{ url('dago-master/setting-menu') }}",
                    ...
                    if(typeof result.data.data !== 'undefined'){
                        ...
                        var html = `<table class='treegrid table' id='sai-treegrid'>
                            ...
                            for(var i=0;i<result.data.data.length;i++){
                                ...
                                <td class='set_nama'>${line[i].nama}<input type='hidden' name='nama[]' value='${line[i].nama}'><input type='hidden' name='jenis_menu[]' value='${line[i].jenis_menu}'></td>
                                <td class='set_link'>${line[i].kode_form}<input type='hidden' name='link[]' value='${line[i].kode_form}'><input type='hidden' class='set_icon' name='icon[]' value='${line[i].icon}'></td>
        ...
        $("#sai-treegrid-modal-form").on("submit", function(event){
            ...
            if(parameter == "edit"){
                var url = "{{ url('dago-master/setting-menu') }}";
            }else{
                var url = "{{url('dago-master/setting-menu')}}";
            }
    ...
    e.preventDefault();
        ...
        $.ajax({
            type: 'POST',
            url: "{{ url('dago-master/setting-menu-move') }}",
            dataType: 'json',
            data: formData,
            async:false,
            contentType: false,
