        ...
        <div class="col-12">
            <div class="card" >
                <div class="card-body pb-3" style="padding-top:1rem;">
                    <h6 style="position:absolute;top: 25px;">Data Kompetensi Dasar</h6>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document" style="max-width:600px">
            <div class="modal-content" style="border-radius:0.75em">
                <div class="modal-header py-0" style="display:block;height:49px">
                    <h6 class="modal-title py-2" style="position: absolute;">Preview Data KD <span id="modal-preview-nama"></span><span id="modal-preview-id" style="display:none"></span><span id="modal-preview-kode" style="display:none"></span> </h6>
    ...
    $('#modalFilter').on('submit','#form-filter',function(e){
        ...
        $.fn.dataTable.ext.search.push(
            function( settings, data, dataIndex ) {
                ...
                if(kode_pp != "" && kode_sem != "" && kode_ta != ""){
                    if(kode_pp == col_kode_pp && kode_sem == col_kode_sem && kode_ta == col_kode_ta){
                        return true;
                    }else{
                        return false;
                    }
                }else if(kode_pp !="" && kode_sem == "" && kode_ta == "") {
    ...
    $("#form-import").validate({
        ...
        submitHandler: function (form) {
            ...
            formData.append('kode_tingkat',$('#kode_tingkat').val());
            ...
            $.ajax({
                ...
                type: 'POST',
                url: "{{ url('sekolah-master/import-kd') }}",
                dataType: 'json',
                data: formData,
                // async:false,
                contentType: false,
                ...
                success:function(result){
                    if(result.data.status){
                        if(result.data.validate){
                            ...
                            var link = "{{ config('api.url').'sekolah/kd-export' }}?kode_lokasi="+kode_lokasi+"&nik_user="+nik_user+"&nik="+nik+"&type=non&kode_pp="+$('#kode_pp').val()+"&kode_tingkat="+$('#kode_tingkat').val()+"&kode_matpel="+$('.info-name_kode_matpel > span ').text()+"&kode_ta="+$('#kode_ta').val()+"&kode_sem="+$('#kode_sem option:selected').text();
