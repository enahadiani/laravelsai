...
<head>
    ...
    <script src="{{ asset('accounting.js') }}"></script>
...
@endif
    ...
    function msgDialog(data){
        switch(data.type){
            ...
            case 'logout':
                ...
                //     window.location.href = "{{ url('sima-auth/logout') }}";
                // }
        ...
        swalWithBootstrapButtons.fire({
            ...
            switch(data.type){
                ...
                case 'logout':
                    if (result.value) {
                        window.localStorage.setItem('logged_in', false);
                        window.localStorage.removeItem('sima-form');
                        window.location.href = "{{ url('sima-auth/logout') }}";
                    } else if (result.dismiss === Swal.DismissReason.cancel) {
                        // console.log('cancel');
                    }                    
    ...
    if(window.localStorage.getItem('sima-form') != "" && window.localStorage.getItem('sima-form') != null && window.localStorage.getItem('sima-form') != "-"){
        var form = window.localStorage.getItem('sima-form');
    }else{
        ...
        $.ajax({
            ...
            error: function(jqXHR, textStatus, errorThrown) {       
                if(jqXHR.status == 422){
                    ...
                    window.location="{{ url('/sima-auth/sesi-habis') }}";
    ...
    function updateNotifRead(){
        $.ajax({
            type: 'POST',
            url: "{{ url('sima-auth/notif-update-status') }}",
            dataType: 'json',
            async:false,
            success:function(result){    
                $('.count').remove();
    ...
    function loadForm(url){
        $.ajax({
            type: 'GET',
            url: "{{ url('sima-auth/cek_session') }}",
            dataType: 'json',
            async:false,
            success:function(result){    
                if(!result.status){
                    window.location.href = "{{ url('sima-auth/sesi-habis') }}";
                }else{
    ...
    function getFormList() {
        $.ajax({
            type:'GET',
            url:"{{url('sima-auth/search-form-list2')}}",
            dataType: 'json',
            async: false,
            success: function(result) {
                if(result.status) {
            ...
            success:function(result){  
                if(result.data.success.status){
                    if(result.data.success.data.length > 0){
                        ...
                        loadForm("{{ url('sima-auth/form')}}/"+form);
                        return false;
                    }
                }else{
    ...
    function loadProfile(){
        loadForm("{{url('sima-auth/form/fProfile')}}");
    }
    
    function loadMenu(){
        $.ajax({
            type: 'GET',
            url: "{{ url('sima-auth/menu') }}",
            dataType: 'json',
            async:false,
            success:function(result){  
                if(result[0].status){
    ...
    $('.sub-menu').on('click','.a_link',function(e){
        ...
        window.localStorage.setItem('sima-form',form);
        $form_aktif = $(this).data('kode_form');
        $('.sub-menu li').removeClass('active');
        $(this).closest('li').addClass('active');
        var url = "{{ url('sima-auth/form')}}/"+form;
        if(form == "" || form == "-"){
            // alert('Form dilock!');
            return false;
        }else{
    ...
    //     source: function (cari, result) {
    //         $.ajax({
    //             url: "{{ url('sima-auth/search-form-list') }}",
    //             data: {cari:cari},            
    //             dataType: "json",
    //             type: "GET",
    ...
    <script>
        ...
        //     console.log('ok');
        //     loadForm("{{ url('sima-auth/form') }}/fMasakun");
    ...
    src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCwdyiC2sZ3B1O2nMdhUy6Z0ljoK3gbA_U&libraries=places">
    </script>
</body>
</html>