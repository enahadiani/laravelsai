<link rel="stylesheet" href="{{ asset('trans.css') }}" />
    ...
    function getKlp(id,pp=null){
        var tmp = id.split(" - ");
        kode = tmp[0];
        var toUrl = "{{ url('esaku-master/setting-rasio-klp') }}";
        $.ajax({
            type: 'GET',
            url: toUrl,
            dataType: 'json',
    ...
    // getDataTypeAhead("{{ url('esaku-master/setting-rasio-neraca') }}","kode_neraca","kode_neraca");
    ...
    function getNeraca(id,target1,target2,target3,jenis){
        ...
        $.ajax({
            type: 'GET',
            url: "{{ url('/esaku-master/setting-rasio-neraca') }}/" + kode,
            dataType: 'json',
            async:false,
            success:function(result){
                if(result.status){
    ...
    var dataTable = generateTable(
        "table-data",
        "{{ url('esaku-master/setting-rasio') }}", 
        [
            {
                "targets": [3],
                "visible": false,
        ...
        switch(id){
            ...
            case 'kode_klp' :
                var settings = {
                    id : id,
                    header : ['Kode', 'Nama'],
                    url : "{{ url('esaku-master/setting-rasio-klp') }}",
                    columns : [
                        { data: 'kode_klp' },
                        { data: 'nama' }
                    ],
        ...
        switch(id){
            case 'kode_neraca[]' :
                ...
                var settings = {
                    id : id,
                    header : ['Kode', 'Nama'],
                    url : "{{ url('esaku-master/setting-rasio-neraca') }}",
                    columns : [
                        { data: 'kode_neraca' },
                        { data: 'nama' }
                    ],
    ...
    $('#form-tambah').validate({
        ...
        submitHandler: function (form) {
            ...
            // $iconLoad.show();
            if(param == "edit"){
                var url = "{{ url('/esaku-master/setting-rasio') }}";
            }else{
                var url = "{{ url('/esaku-master/setting-rasio') }}";
            }
                $.ajax({
    ...
    $('#table-data tbody').on('click','td',function(e){
        if($(this).index() != 3){
            ...
            $.ajax({
                type: 'GET',
                url: "{{ url('/esaku-master/setting-rasio-detail') }}",
                dataType: 'json',
                data:{kode_rasio: id},
                async:false,
                success:function(result){
    ...
    function hapusData(id){
        $.ajax({
            type: 'DELETE',
            url: "{{ url('esaku-master/setting-rasio') }}",
            dataType: 'json',
            data:{kode_rasio:id},
            async:false,
            success:function(result){
