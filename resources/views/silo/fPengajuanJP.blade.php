...
<script type="text/javascript">
    ...
    function generateDok(tanggal,kode_pp,kode_kota){
        $.ajax({
            type: 'GET',
            url: "{{ url('silo-trans/generate-dok-juspo') }}",
            dataType: 'json',
    ...
    function printPreview(id, juskeb) {
        $.ajax({
            type: 'GET',
            url: "{{ url('apv/juspo_preview') }}/" + juskeb + "/" + id,
            dataType: 'json',
            async:false,
            success:function(res) {
                var result = res.data
        ...
        $('#saku-datatable').on('click','#btn-print',function(e) {
            var id = $(this).closest('tr').find('td').eq(0).html();
            var juskeb = dataTable.row($(this).closest('tr')).data()
            printPreview(id, juskeb.id);
        });
