    ...
    function getNeraca(id,target1,target2,target3,jenis){
        ...
        $.ajax({
            type: 'GET',
            url: "{{ url('/yakes-trans/pendukung-neraca') }}",
            dataType: 'json',
            data:{kode_neraca:id},
            async:false,
            success:function(result){
    ...
    $('#form-tambah').validate({
        ...
        submitHandler: function (form) {
            ...
            // $iconLoad.show();
            if(param == "edit"){
                var url = "{{ url('/yakes-trans/pendukung') }}";
            }else{
                var url = "{{ url('/yakes-trans/pendukung') }}";
            }
                $.ajax({
    ...
    $('#table-data tbody').on('click','td',function(e){
        if($(this).index() != 2){
            ...
            $.ajax({
                type: 'GET',
                url: "{{ url('/yakes-trans/pendukung-detail') }}",
                dataType: 'json',
                data:{kode_dash: id},
                async:false,
                success:function(result){
