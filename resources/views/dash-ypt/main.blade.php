...
<nav class="navbar fixed-top px-0 py-0">
    ...
    function msgDialog(data){
        ...
        switch(data.type){
            ...
            case 'logout':
                ...
                //     window.location.href = "{{ url('dash-ypt/logout') }}";
                // }
    ...
    function updateNotifRead(){
        $.ajax({
            type: 'POST',
            url: "{{ url('dash-ypt/notif-update-status') }}",
            dataType: 'json',
            async:false,
            success:function(result){    
                $('.count').remove();
            ...
            success:function(result){  
                if(result.data.success.status){
                    if(result.data.success.data.length > 0){
                        ...
                        loadForm("{{ url('dash-ypt/form')}}/"+form);
                        return false;
                    }
                }else{
    ...
    function loadProfile(){
        loadForm("{{url('dash-ypt/form/fProfile')}}");
    }
    
    function loadMenu(){
        $.ajax({
            type: 'GET',
            url: "{{ url('dash-ypt/menu') }}",
            dataType: 'json',
            async:false,
            success:function(result){  
                if(result[0].status){
    ...
    $('.to-home').click(function(){
        if(form != "" || form != "-"){
            ...
            loadForm("{{ url('dash-ypt/form') }}/blankform");
        }
    });
    ...
    if (mode == "dark") {
        ...
        var url_logo_mob = "{{ asset('img/ypt.jpeg') }}";
        $('.logo').css({
