...
@endif
    ...
    function msgDialog(data){
        switch(data.type){
            ...
            case 'logout':
                ...
                //     window.location.href = "{{ url('rtrw-auth/logout') }}";
                // }
        ...
        swalWithBootstrapButtons.fire({
            ...
            switch(data.type){
                ...
                case 'logout':
                    if (result.value) {
                        window.localStorage.setItem('logged_in', false);
                        window.localStorage.removeItem('rtrw-form');
                        window.location.href = "{{ url('rtrw-auth/logout') }}";
                    } else if (result.dismiss === Swal.DismissReason.cancel) {
                        // console.log('cancel');
                    }                    
    ...
    // if(window.localStorage.getItem('rtrw-form') != "" && window.localStorage.getItem('rtrw-form') != null && window.localStorage.getItem('rtrw-form') != "-"){
    //     var form = window.localStorage.getItem('rtrw-form');
    // }else{
        ...
        $.ajax({
            ...
            error: function(jqXHR, textStatus, errorThrown) {       
                if(jqXHR.status == 422){
                    ...
                    window.location="{{ url('/rtrw-auth/sesi-habis') }}";
    ...
    function updateNotifRead(){
        $.ajax({
            type: 'POST',
            url: "{{ url('rtrw-auth/notif-update-status') }}",
            dataType: 'json',
            async:false,
            success:function(result){    
                $('.count').remove();
    ...
    var channel = pusher.subscribe('sairtrw-channel-'+userNIK);
    channel.bind('sairtrw-event', function(data) {
        getNotif();
    ...
    function getFormList() {
        $.ajax({
            type:'GET',
            url:"{{url('rtrw-auth/search-form-list2')}}",
            dataType: 'json',
            async: false,
            success: function(result) {
                if(result.status) {
            ...
            success:function(result){  
                if(result.data.success.status){
                    if(result.data.success.data.length > 0){
                        ...
                        loadForm("{{ url('rtrw-auth/form')}}/"+form);
                        return false;
                    }
                }else{
    ...
    function loadProfile(){
        loadForm("{{url('rtrw-auth/form/fProfile')}}");
    }
    
    function loadMenu(){
        $.ajax({
            type: 'GET',
            url: "{{ url('rtrw-auth/menu') }}",
            dataType: 'json',
            async:false,
            success:function(result){  
                if(result[0].status){
    ...
    $('.sub-menu').on('click','.a_link',function(e){
        ...
        window.localStorage.setItem('rtrw-form',form);
    ...
    //     source: function (cari, result) {
    //         $.ajax({
    //             url: "{{ url('rtrw-auth/search-form-list') }}",
    //             data: {cari:cari},            
    //             dataType: "json",
    //             type: "GET",
    ...
    <script>
        ...
        //     console.log('ok');
        //     loadForm("{{ url('rtrw-auth/form') }}/fMasakun");
