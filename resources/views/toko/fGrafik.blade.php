    ...
    function getKlp(id,pp=null){
        var tmp = id.split(" - ");
        kode = tmp[0];
        var toUrl = "{{ url('esaku-master/setting-grafik-klp') }}";
        $.ajax({
            type: 'GET',
            url: toUrl,
            dataType: 'json',
    ...
    getDataTypeAhead("{{ url('esaku-master/helper-fs') }}","kode_fs","kode_fs");
    getDataTypeAhead("{{ url('esaku-master/setting-grafik-neraca') }}","kode_neraca","kode_neraca");
    ...
    function getNeraca(id,target1,target2,target3,jenis){
        ...
        $.ajax({
            type: 'GET',
            url: "{{ url('/esaku-master/setting-grafik-neraca') }}/" + kode,
            dataType: 'json',
            async:false,
            success:function(result){
                if(result.status){
    ...
    function getFS(id,target1,target2,jenis){
        ...
        $.ajax({
            type: 'GET',
            url: "{{ url('/esaku-master/helper-fs') }}/"+ kode,
            dataType: 'json',
            async:false,
            success:function(result){
                if(typeof result.daftar !== 'undefined' && result.daftar.length>0){
    ...
    var dataTable = generateTable(
        "table-data",
        "{{ url('esaku-master/setting-grafik') }}", 
        [
            {
                "targets": [5],
                "visible": false,
    ...
    $(".div-kode_klp").showFilter({
        ...
        url: "{{ url('esaku-master/setting-grafik-klp') }}",
        header:['Kode','Nama'],
        columns:[
            { data: 'kode_klp' },
            { data: 'nama' }
    ...
    function addRowGrid(param) {
        ...
        $(".search-neracake"+no).showFilter({
            title: 'Daftar Neraca',
            url: "{{ url('esaku-master/setting-grafik-neraca') }}",
            header:['Kode','Nama'],
            columns:[
                        { data: 'kode_neraca' },
                        { data: 'nama' }
        ...
        $(".search-fske"+no).showFilter({
            title: 'Daftar FS',
            url: "{{ url('esaku-master/helper-fs') }}",
            header:['Kode','Nama'],
            columns:[
                        { data: 'kode_fs' },
                        { data: 'nama' }
    ...
    $('#form-tambah').validate({
        ...
        submitHandler: function (form) {
            ...
            // $iconLoad.show();
            if(param == "edit"){
                var url = "{{ url('/esaku-master/setting-grafik') }}";
            }else{
                var url = "{{ url('/esaku-master/setting-grafik') }}";
            }
    ...
    $('#table-data tbody').on('click','td',function(e){
        if($(this).index() != 5){
            ...
            $.ajax({
                type: 'GET',
                url: "{{ url('/esaku-master/setting-grafik-detail') }}",
                dataType: 'json',
                data:{kode_grafik: id},
                async:false,
                success:function(result){
    ...
    function hapusData(id){
        $.ajax({
            type: 'DELETE',
            url: "{{ url('esaku-master/setting-grafik') }}",
            dataType: 'json',
            data:{kode_grafik:id},
            async:false,
            success:function(result){
