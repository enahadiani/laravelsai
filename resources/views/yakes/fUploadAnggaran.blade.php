                    ...
                    <div class="card-body form-body" style='background:#f8f8f8;padding: 0 !important;border-bottom-left-radius: .75rem;border-bottom-right-radius: .75rem;'>
                        ...
                        <div class="card mt-3" style='border-top-left-radius:0;border-top-right-radius:0'>
                            <div class="card-body">
                                ...
                                <li class="nav-item"> <a class="nav-link active" data-toggle="tab" href="#data-agg" role="tab" aria-selected="true"><span class="hidden-xs-down">Data Anggaran</span></a> </li>
                                </ul>
    ...
    $('#form-tambah').validate({
        ...
        submitHandler: function (form) {
            ...
            // $iconLoad.show();
            if(param == "edit"){
                var url = "{{ url('yakes-trans/anggaran') }}";
            }else{
                var url = "{{ url('yakes-trans/anggaran') }}";
            }
            ...
            if(jumdet <= 1){
                alert('Transaksi tidak valid. Data Angagran tidak boleh kosong ');
            }else{
                $.ajax({
                    type: 'POST',
                    url: url,
    ...
    // ENTER FIELD FORM
    $('#tahun,#keterangan,#file').keydown(function(e){
        var code = (e.keyCode ? e.keyCode : e.which);
        var nxt = ['tahun','keterangan','file'];
        if (code == 13 || code == 40) {
            e.preventDefault();
    ...
    $("#form-import").validate({
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                ...
                type: 'POST',
                url: "{{ url('yakes-trans/anggaran-upload') }}",
                dataType: 'json',
                data: formData,
                // async:false,
                contentType: false,
    ...
    $('#process-upload').click(function(e){
        $.ajax({
            type: 'GET',
            url: "{{ url('yakes-trans/anggaran-load') }}",
            dataType: 'json',
    ...
    $('#download-template').click(function(){
        ...
        var nik_user = "{{ Session::get('nikUser') }}";
        var nik = "{{ Session::get('userLog') }}";
        var link = "{{ config('api.url').'yakes-trans/anggaran-export' }}?kode_lokasi="+kode_lokasi+"&nik_user="+nik_user+"&nik="+nik+"&type=template";
