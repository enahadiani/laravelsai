...
<div class="container-fluid mt-3">
    <div class="row">
        <div class="col-12 bank-det">
            <h6 class="mb-0 bold">Bank</h6>
        ...
        <div class="col-lg-7 col-12 mb-4">
            <div class="card dash-card">
                <div class="card-header">
                    <h6 class="card-title mb-0">Rincian Bank</h6>
                </div>
                <div class="card-body">
                    <table class='no-border' id='tableBank' style="width:100%">
                        <thead>
                            <tr>
...
function getMSBank(periode=null){
    $.ajax({
        type:"GET",
        url:"{{ url('/mobile-dash/ms-bank') }}",
        ...
        success:function(result){
            ...
            for(var i=0;i<result.data.data.length;i++){
                ...
                html+=`<tr>
                    <td style=''>`+line.nama+`</td>
                    <td class='text-right bold'>`+sepNum(line.so_akhir)+`</td>
                     </tr>`;
                     total+= parseFloat(line.so_akhir);
                            
            }
            html+=`<tr>
                    <td style='bold'>Total</td>
                    <td class='text-right bold'>`+sepNum(total)+`</td>
                     </tr>`;
            $('#tableBank tbody').html(html);
        },
        error: function(jqXHR, textStatus, errorThrown) {       
            if(jqXHR.status == 422){
                var msg = jqXHR.responseText;
...
$('.bank-det').on('click','#btnBack',function(e){
    e.preventDefault();
