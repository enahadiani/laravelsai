                        ...
                        <div class="px-4 pb-4 pt-2">
                            <form id="form-filter">
                                ...
                                <div id="inputFilter">
                                    ...
                                    <x-inp-filter kode="kode_fakultas" nama="Kode Fakultas" selected="3" :option="array('1','3')"/>
...
<script>
        ...
        $.ajax({
            ...
            success:function(result){   
                if(result.status){
                    ...
                    $('#kode_fakultas-from').val(result.kode_fakultas);
                    $kode_fakultas = {
                        type : "=",
                        from : result.kode_fakultas,
                        fromname : result.nama_fakultas,
                        to : "",
                        toname : "",
                    }
    ...
    $('#form-filter').submit(function(e){
        ...
        if($output.from == "Laporan"){
            xurl = "{{ url('dash-telu/form/rptLabaRugiAggFak2') }}";
        }else{
            xurl = "{{ url('dash-telu/form/rptLabaRugiAggFak2Grid') }}";
        }
    ...
    $("#sai-rpt-pdf").click(function(e) {
        e.preventDefault();
        var link = "{{ url('telu-report/lap-labarugi-agg-fak2-pdf') }}?periode[]="+$periode.type+"&periode[]="+$periode.from+"&periode[]="+$periode.to+"&kode_fs[]="+$kode_fs.type+"&kode_fs[]="+$kode_fs.from+"&kode_fs[]="+$kode_fs.to+"&output[]="+$output.type+"&output[]="+$output.from+"&output[]="+$output.to+"&kode_fakultas[]="+$kode_fakultas.type+"&kode_fakultas[]="+$kode_fakultas.from+"&kode_fakultas[]="+$kode_fakultas.to+"&kode_fakultas[]="+$kode_fakultas.fromname+"&kode_fakultas[]="+$kode_fakultas.toname;
    ...
    $('.navigation-lap').on('click', '#btn-back', function(e){
        ...
        if(aktif == "neraca-lajur"){
            xurl = "dash-telu/form/rptLabaRugiAggFak2";
            $formData.delete('back');
