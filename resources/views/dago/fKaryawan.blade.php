...
<div class="container-fluid mt-3" style="font-size:13px">
    ...
    function showFilter(param,target1,target2){
        ...
        case 'kode_pp': 
            header = ['Kode', 'Nama'];
            var toUrl = "{{ url('dago-master/unit') }}";
                var columns = [
                    { data: 'kode_pp' },
                    { data: 'nama' }
                ];
    ...
    $('#saku-form').on('submit', '#form-tambah', function(e){
        ...
        var id = $('#id').val();
        if(parameter == "edit"){
            var url = "{{ url('dago-master/karyawan-ubah') }}/"+id;
            var pesan = "updated";
        }else{
            var url = "{{ url('dago-master/karyawan') }}";
            var pesan = "saved";
        }
    ...
    $('#saku-datatable').on('click','#btn-delete',function(e){
        ...
        confirmButtonText: 'Yes, delete it!'
            ...
            if (result.value) {
                ...
                $.ajax({
                    type: 'DELETE',
                    url: "{{ url('dago-master/karyawan') }}/"+id,
                    dataType: 'json',
                    async:false,
                    success:function(result){
                        if(result.data.status){
