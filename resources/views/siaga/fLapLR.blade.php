<link rel="stylesheet" href="{{ asset('report-new.css?version=_').time() }}" />
    <div class="row" id="saku-filter">
        <div class="col-12">
            <div class="card" >
                <x-report-header judul="Laporan Laba Rugi Anggaran"/>
    ...
    <script>
        ...
        function loadFilterDefault(){
            $.ajax({
                type: 'GET',
                url: "{{ url('siaga-report/filter-default-labarugi-agg') }}",
                dataType: 'json',
                async:false,
                success:function(result){   
                    if(result.status){
                        ...
                        $periode = {
                            type : "=",
                            from : result.periode,
                            fromname : result.periode,
                            to : "",
                            toname : "",
                        }

                        generateRptFilter('#inputFilter',{
                            kode : ['periode','kode_fs'],
                            nama : ['Periode','Kode FS'],
                            header : [['Periode'],['Kode', 'Nama']],
                            headerpilih : [['Periode','Action'],['Kode', 'Nama','Action']],
                            columns: [
                                [
                                    { data: 'periode' }
                                ],[
                            ...
                            url :["{{ url('siaga-report/filter-periode-keu') }}","{{ url('siaga-report/filter-fs') }}"],
                            parameter:[],
                            orderby:[[[0,"desc"]],[],[]],
        ...
        $('.navigation-lap').on('click', '#btn-back', function(e){
            ...
            if(aktif == "neraca-lajur"){
                xurl = "dash-telu/form/rptLabaRugiAgg";
                $formData.delete('back');
        ...
        $('#modalEmail').on('submit','#formEmail',function(e){
            ...
            formData.append("subject","Laporan Laba Rugi Anggaran");
            $.ajax({
                type: 'POST',
