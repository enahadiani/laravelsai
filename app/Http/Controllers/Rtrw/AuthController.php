...
class AuthController extends Controller
{
    ...
    public function index()
    {
        ...
        if(!Session::get('login')){
            return redirect('rtrw-auth/login')->with('alert','Kamu harus login dulu');
        }
        else{
            return view('rtrw-baru.main');
        }
        ...
        try {
            ...
            if ($response->getStatusCode() == 200) { // 200 OK
                ...
                if($data["message"] == "success"){
                    ...
                    return redirect('rtrw-auth/');
                }else{
                    return redirect('rtrw-auth/login')->with('alert','Password atau NIK, Salah !');
                }
            ...
            return redirect('rtrw-auth/login')->with('alert',$res["message"]);
        }
        
    }

    public function login(){
        return view('rtrw-baru.login');
    }

    public function logout(){
        Session::flush();
        return redirect('rtrw-auth/login')->with('alert','Kamu sudah logout');
    }
    ...
    public function getMenu(){
        try {
            $client = new Client();
            $kodemenu = Session::get('kodeMenu');
            $response = $client->request('GET',  config('api.url').'rtrw/menu-web/'.$kodemenu,[
                'headers' => [
                    'Authorization' => 'Bearer '.Session::get('token'),
                    'Accept'     => 'application/json',
                    ]
