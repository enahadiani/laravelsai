...
<head>
    <meta charset="UTF-8">
    <title>DAPEN - Admin Dashboard</title>
    ...
    <style>
        ...
        .logo{
            background:url("{{ asset('img/dapen.png') }}") no-repeat;
    ...
    <script src="{{ asset('asset_dore/js/sai.js') }}"></script>
        ...
        <div class="navbar-right" >
            <div class="user d-inline-block mr-3 dropdown">
                <button class="btn btn-empty p-0" id="btn-admin" type="button" data-toggle="dropdown" aria-haspopup="true"
                    ...
                    <img alt="Profile Picture" src="{{ asset('img/user.png') }}" style="width:40px;height:40px"/>
                    @else
                    ...
                    <a href="#" class="dropdown-profile">
                        <div style="height: 45px;padding: 0 1rem;">
                            <span id="adminProfilePhoto">
                                @if (Session::get('foto') == "" || Session::get('foto') == "-" )
                                <img alt="Profile Picture" class="imgprofile ml-0" src="{{ asset('img/user.png') }}" style="width:40px;height:40px"/>
                                @else
            ...
            <div class="header-icons d-inline-block align-middle mr-4">
                <div class="dropdown position-relative d-inline-block">
                    ...
                    <div class="dropdown-menu dropdown-menu-right position-absolute py-0 mt-0" id="notificationDropdown2" style="width:300px;">
                        ...
                        <div class="notif-body" style="height:280px;position:relative">
                        </div>
...
class BottomSheet {
    ...
    function updateNotifRead(){
        $.ajax({
            type: 'POST',
            url: "{{ url('rkap-auth/notif-update-status') }}",
            dataType: 'json',
            async:false,
            success:function(result){    
                $('.count').remove();
    ...
    var channel = pusher.subscribe('sairkap-channel-'+userNIK);
    channel.bind('sairkap-event', function(data) {
        getNotif();
    ...
    function getFormList() {
        $.ajax({
            type:'GET',
            url:"{{url('rkap-auth/search-form-list2')}}",
            dataType: 'json',
            async: false,
            success: function(result) {
                if(result.status) {
            ...
            success:function(result){  
                if(result.data.success.status){
                    if(result.data.success.data.length > 0){
                        ...
                        loadForm("{{ url('rkap-auth/form')}}/"+form);
                        return false;
                    }
                }else{
    ...
    function loadProfile(){
        loadForm("{{url('rkap-auth/form/fProfile')}}");
    }
    
    function loadMenu(){
        $.ajax({
            type: 'GET',
            url: "{{ url('rkap-auth/menu') }}",
            dataType: 'json',
            async:false,
            success:function(result){  
                if(result[0].status){
    ...
    //     source: function (cari, result) {
    //         $.ajax({
    //             url: "{{ url('rkap-auth/search-form-list') }}",
    //             data: {cari:cari},            
    //             dataType: "json",
    //             type: "GET",
    ...
    function setHeightForm(){
        ...
        if($('.top-menu').length > 0){
            $('.top-menu').css('width',$('.container-fluid').width()+'px');
        }
