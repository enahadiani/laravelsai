            ...
            if(result.status){
                switch(index){
                    ...
                    if(result.data[0].foto == "-" || result.data[0].foto == "" || result.data[0].foto == undefined){
                        ...
                        <img alt="Profile" src="{{ asset('img/user.png') }}" class="img-thumbnail card-img social-profile-img"  style="width:120px;height:120px;border-radius: 50%;">
                        `;
                    }else{
...
function initDash(){
    loadService('profile','GET',"sima-auth/profile"); 
}
initDash();
...
$('#form-ubahPass').on('submit', function(e){
    e.preventDefault();
        ...
        var url = "sima-auth/update-password";
        var pesan = "saved";
...
$('.crop_image').click(function(event){
    $image_crop.croppie('result', {
        ...
        if(tipe == 'foto'){
            var toUrl = "{{ url('sima-auth/update-foto') }}";
        }else{
            var toUrl = "{{ url('sima-auth/update-background') }}";
        }
