...
class AuthController extends Controller
{
    ...
    public function cek_auth(Request $request){
        ...
        try {
            $client = new Client();
            $response = $client->request('POST',  config('api.url').'gis-auth/login',[
                'form_params' => [
            ...
            if ($response->getStatusCode() == 200) { // 200 OK
                ...
                if($data["message"] == "success"){
                    ...
                    $response2 = $client->request('GET',  config('api.url').'gis-auth/profile',[
                        'headers' => [
                    ...
                    if ($response2->getStatusCode() == 200) { // 200 OK
                        ...
                        if(count($res) > 0){
                            ...
                            Session::put('foto',"-");
                            Session::put('logo',"-");
                            Session::put('periode',(isset($data2["kode_fs"][0]["kode_fs"]) ? $data2["kode_fs"][0]["kode_fs"] : ""));
                            Session::put('kode_fs',(isset($data2["kode_fs"][0]["kode_fs"]) ? $data2["kode_fs"][0]["kode_fs"] : ""));
                        }
                    }
                    
                    return redirect('sima-auth/');
                }else{
                    return redirect('sima-auth/login')->with('alert','Password atau NIK, Salah !');
                }
            ...
            return redirect('sima-auth/login')->with('alert',$message);
        }
        
    }

    public function login(){
        return view('sima.login');
    }

    public function register(){
        return view('sima.register');
    }

    public function logout(){
        Session::flush();
        return redirect('sima-auth/login')->with('status','logout');
    }
    ...
    public function getMenu(){
        try {
            ...
            $response = $client->request('GET',  config('api.url').'gis-auth/menu/'.$kodemenu,[
                'headers' => [
                ...
                $main_menu = $data['data'];
                    ...
                    if(count($main_menu) > 0){
                        for($i=0; $i<count($main_menu); $i++){
                            ...
                            $forms = str_replace("_","/", $main_menu[$i]['id_form']);
    ...
    public function getProfile(){
        try {
            $client = new Client();
            $response = $client->request('GET',  config('api.url').'gis-auth/profile',[
                'headers' => [
                    'Authorization' => 'Bearer '.Session::get('token'),
                    'Accept'     => 'application/json',
                ]
    ...
    public function updatePassword(Request $request){
        ...
        try {
            $client = new Client();
            $response = $client->request('POST',  config('api.url').'gis-auth/update-password',[
                'headers' => [
                    'Authorization' => 'Bearer '.Session::get('token'),
                    'Accept'     => 'application/json',
                ],
    ...
    public function updatePhoto(Request $request){
        ...
        try {
            ...
            $client = new Client();
            $response = $client->request('POST',  config('api.url').'gis-auth/update-foto',[
                'headers' => [
                    'Authorization' => 'Bearer '.Session::get('token'),
                    'Accept'     => 'application/json',
                ],
    ...
    public function updateBackground(Request $request){
        ...
        try {
            ...
            $client = new Client();
            $response = $client->request('POST',  config('api.url').'gis-auth/update-background',[
                'headers' => [
                    'Authorization' => 'Bearer '.Session::get('token'),
                    'Accept'     => 'application/json',
                ],
    ...
    public function searchForm(Request $request){
        ...
        try {
            $client = new Client();
            $response = $client->request('POST',  config('api.url').'gis-auth/search-form',[
                'headers' => [
                    'Authorization' => 'Bearer '.Session::get('token'),
                    'Accept'     => 'application/json',
                ],
    ...
    public function searchFormList(Request $request){
        try {
            $client = new Client();
            $response = $client->request('GET',  config('api.url').'gis-auth/search-form-list',[
                'headers' => [
                    'Authorization' => 'Bearer '.Session::get('token'),
                    'Accept'     => 'application/json',
                ],
