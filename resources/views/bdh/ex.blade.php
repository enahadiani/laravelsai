...
function addRow(param){
    if(param == "copy"){
        ...
        var kode_drk = $('#input-grid tbody tr.selected-row').find(".inp-drk").val();
        var nama_drk = $('#input-grid tbody tr.selected-row').find('.inp-nama_drk').val();
    }else{
...
$('#input-grid').on('keydown','.inp-kode, .inp-nama, .inp-dc, .inp-ket, .inp-nilai, .inp-pp, .inp-drk, .inp-nama_pp',function(e){
    var code = (e.keyCode ? e.keyCode : e.which);
    var nxt = ['.inp-kode','.inp-nama','.inp-dc','.inp-ket','.inp-nilai','.inp-pp','.inp-drk','.inp-nama_pp'];
    var nxt2 = ['.td-kode','.td-nama','.td-dc','.td-ket','.td-nilai','.td-pp','.td-drk','.td-nama_pp'];
    if (code == 13 || code == 9) {
        e.preventDefault();
...
function hideUnselectedRow() {
    $('#input-grid > tbody > tr').each(function(index, row) {
        if(!$(row).hasClass('selected-row')) {
            ...
            var kode_drk = $('#input-grid > tbody > tr:eq('+index+') > td').find(".inp-drk").val();
            ...
            $('#input-grid > tbody > tr:eq('+index+') > td').find(".inp-drk").val(kode_drk);
            $('#input-grid > tbody > tr:eq('+index+') > td').find(".td-drk").text(kode_drk);
            $('#input-grid > tbody > tr:eq('+index+') > td').find(".inp-nama_drk").val(nama_drk);
            $('#input-grid > tbody > tr:eq('+index+') > td').find(".td-nama_drk").text(nama_drk);
            ...
            $('#input-grid > tbody > tr:eq('+index+') > td').find(".inp-drk").hide();
            $('#input-grid > tbody > tr:eq('+index+') > td').find(".td-drk").show();
            $('#input-grid > tbody > tr:eq('+index+') > td').find(".search-drk").hide();
            $('#input-grid > tbody > tr:eq('+index+') > td').find(".inp-nama_drk").hide();
            $('#input-grid > tbody > tr:eq('+index+') > td').find(".td-nama_drk").show();
        }
    })
}
...
$('#input-grid').on('click', 'td', function(){
    ...
    if(idx == 0){
        ...
        if($(this).hasClass('px-0 py-0 aktif')){
            ...
            var kode_pp = $(this).parents("tr").find(".inp-drk").val();
            ...
            $(this).parents("tr").find(".inp-drk").val(kode_pp);
            $(this).parents("tr").find(".td-drk").text(kode_pp);
            if(idx == 8){
...
$('#input-grid').on('keypress', '.inp-drk', function(e){
    var this_index = $(this).closest('tbody tr').index();
    if (e.which == 42) {
        e.preventDefault();
        if($("#input-grid tbody tr:eq("+(this_index - 1)+")").find('.inp-drk').val() != undefined){
            $(this).val($("#input-grid tbody tr:eq("+(this_index - 1)+")").find('.inp-drk').val());
        }else{
            $(this).val('');
        }
    }
