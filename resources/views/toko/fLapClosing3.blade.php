...
<div class="row" style="">
        <div style="z-index: 1;position: fixed;right: auto;left: auto;margin-right: 15px;margin-left: 25px;margin-top:15px" class="col-sm-12" id="subFixbar">
            <div class="card " id="sai-rpt-filter-box;" style="padding:10px;">
                <div class="card-body" style="padding: 0px;">
                    <h4 class="card-title pl-1" style="font-size:16px"><i class='fas fa-file'></i> Laporan Closing</h4>
                    <hr>
                    <form id="formFilter">
    ...
    function getPeriode() {
        $.ajax({
            type:'GET',
            url:"{{url('toko-report/filter-periode-close')}}",
            dataType: 'json',
            async: false,
    ...
    function getNik() {
        $.ajax({
            type:'GET',
            url:"{{url('toko-report/filter-nik-close')}}",
            dataType: 'json',
            async: false,
            success: function(result) {
                if(result.status) {
    ...
    function getNoBukti() {
        $.ajax({
            type:'GET',
            url:"{{url('toko-report/filter-bukti-close')}}",
            dataType: 'json',
            async: false,
            success: function(result) {
                if(result.status) {
                    for(var i=0;i<result.daftar.length;i++){
                        var line = result.daftar[i];
                        $('#no_bukti').append(`<option value='${line.no_bukti}'>${line.no_bukti}</option>`)
                    }
                }else if(!result.status && result.daftar.message == "Unauthorized"){
                    Swal.fire({
                        title: 'Session telah habis',
    ...
    $('.card-body').on('submit', '#formFilter', function(e){
        ...
        xurl = "{{ url('/toko-auth/form')}}/rptClosing3";
