    ...
    function getKlp(id,pp=null){
        var tmp = id.split(" - ");
        kode = tmp[0];
        var toUrl = "{{ url('dash-tarbak-trans/setting-grafik-klp') }}";
        $.ajax({
            type: 'GET',
            url: toUrl,
            dataType: 'json',
    ...
    getDataTypeAhead("{{ url('dash-tarbak-trans/setting-grafik-neraca') }}","kode_neraca","kode_neraca");
    ...
    function getNeraca(id,target1,target2,target3,jenis){
        ...
        $.ajax({
            type: 'GET',
            url: "{{ url('/dash-tarbak-trans/setting-grafik-neraca') }}/" + kode,
            dataType: 'json',
            async:false,
            success:function(result){
                if(result.status){
    ...
    var dataTable = generateTable(
        "table-data",
        "{{ url('dash-tarbak-trans/setting-grafik') }}", 
        [
            {
                "targets": 0,
        ...
        switch(id){
            case 'kode_klp' :
                var settings = {
                    id : id,
                    header : ['Kode', 'Nama'],
                    url : "{{ url('dash-tarbak-trans/setting-grafik-klp') }}",
                    columns : [
                        { data: 'kode_klp' },
                        { data: 'nama' }
                    ],
        ...
        switch(id){
            ...
            case 'kode_neraca[]' :
                ...
                var settings = {
                    id : id,
                    header : ['Kode', 'Nama'],
                    url : "{{ url('dash-tarbak-trans/setting-grafik-neraca') }}",
                    columns : [
                        { data: 'kode_neraca' },
                        { data: 'nama' }
                    ],
    ...
    $('#form-tambah').validate({
        ...
        submitHandler: function (form) {
            ...
            // $iconLoad.show();
            if(param == "edit"){
                var url = "{{ url('/dash-tarbak-trans/setting-grafik') }}";
            }else{
                var url = "{{ url('/dash-tarbak-trans/setting-grafik') }}";
            }
    ...
    $('#table-data tbody').on('click','td',function(e){
        if($(this).index() != 5){
            ...
            $.ajax({
                type: 'GET',
                url: "{{ url('/dash-tarbak-trans/setting-grafik-detail') }}",
                dataType: 'json',
                data:{kode_grafik: id},
                async:false,
                success:function(result){
    ...
    function hapusData(id){
        $.ajax({
            type: 'DELETE',
            url: "{{ url('dash-tarbak-trans/setting-grafik') }}",
            dataType: 'json',
            data:{kode_grafik:id},
            async:false,
            success:function(result){
