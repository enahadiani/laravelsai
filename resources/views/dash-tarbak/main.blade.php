...
<meta charset="UTF-8">
    ...
    <!-- <script src="https://code.highcharts.com/modules/exporting.js"></script> -->
    <script src="{{ asset('dash-asset/dash-tarbak/circle/circle-progress.js') }}"></script>
...
<nav class="navbar fixed-top px-0 py-0">
    ...
    function msgDialog(data){
        ...
        switch(data.type){
            ...
            case 'logout':
                ...
                //     window.location.href = "{{ url('dash-tarbak/logout') }}";
                // }
    ...
    function updateNotifRead(){
        $.ajax({
            type: 'POST',
            url: "{{ url('dash-tarbak/notif-update-status') }}",
            dataType: 'json',
            async:false,
            success:function(result){    
                $('.count').remove();
    ...
    function loadForm(url){
        $.ajax({
            type: 'GET',
            url: "{{ url('dash-tarbak/cek_session') }}",
            dataType: 'json',
            async:false,
            success:function(result){    
                if(!result.status){
    ...
    function getFormList() {
        $.ajax({
            type:'GET',
            url:"{{url('dash-tarbak/search-form-list2')}}",
            dataType: 'json',
            async: false,
            success: function(result) {
                if(result.status) {
    ...
    function searchForm(cari){
        $.ajax({
            type: 'POST',
            url: "{{ url('dash-tarbak/search-form') }}",
            dataType: 'json',
            data:{'cari':cari},
            async:false,
            success:function(result){  
                if(result.data.success.status){
                    if(result.data.success.data.length > 0){
                        ...
                        loadForm("{{ url('dash-tarbak/form')}}/"+form);
                        return false;
                    }
                }else{
    ...
    function loadProfile(){
        loadForm("{{url('dash-tarbak/form/fProfile')}}");
    }
    
    function loadMenu(){
        $.ajax({
            type: 'GET',
            url: "{{ url('dash-tarbak/menu') }}",
            dataType: 'json',
            async:false,
            success:function(result){  
                if(result[0].status){
    ...
    $('.sub-menu').on('click','.a_link',function(e){
        ...
        var url = "{{ url('dash-tarbak/form')}}/"+form;
        console.log(url);
        if(form == "" || form == "-"){
            // alert('Form dilock!');
            return false;
    ...
    //     source: function (cari, result) {
    //         $.ajax({
    //             url: "{{ url('dash-tarbak/search-form-list') }}",
    //             data: {cari:cari},            
    //             dataType: "json",
    //             type: "GET",
    ...
    $('.to-home').click(function(){
        if(form != "" || form != "-"){
            ...
            loadForm("{{ url('dash-tarbak/form') }}/blankform");
        }
    });
    ...
    if (mode == "dark") {
        ...
        var url_logo = "{{ asset('img/tarbak30x100.png') }}";
        var url_logo_mob = "{{ asset('img/tarbak30x100.png') }}";
        $('.logo').css({
