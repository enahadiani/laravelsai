...
class BottomSheet {
        ...
        swalWithBootstrapButtons.fire({
            ...
            switch(data.type){
                ...
                case 'logout':
                    if (result.value) {
                        window.localStorage.setItem('logged_in', false);
                        window.location.href = "{{ url('admjava-auth/logout') }}";
                    } else if (result.dismiss === Swal.DismissReason.cancel) {
                        // console.log('cancel');
                    }                    
                break;
        ...
        $.ajax({
            ...
            error: function(jqXHR, textStatus, errorThrown) {       
                if(jqXHR.status == 422){
                    ...
                    window.location="{{ url('/admjava-auth/sesi-habis') }}";
    ...
    function updateNotifRead(){
        $.ajax({
            type: 'POST',
            url: "{{ url('admjava-auth/notif-update-status') }}",
            dataType: 'json',
            async:false,
            success:function(result){    
                $('.count').remove();
    ...
    function loadForm(url){
        $.ajax({
            type: 'GET',
            url: "{{ url('admjava-auth/cek_session') }}",
            dataType: 'json',
            async:false,
            success:function(result){    
                if(!result.status){
    ...
    function searchForm(cari){
        $.ajax({
            type: 'POST',
            url: "{{ url('admjava-auth/search-form') }}",
            dataType: 'json',
            data:{'cari':cari},
            async:false,
            success:function(result){  
                if(result.data.success.status){
                    if(result.data.success.data.length > 0){
                        ...
                        loadForm("{{ url('admjava-auth/form')}}/"+form);
                        return false;
                    }
                }else{
    ...
    function loadProfile(){
        loadForm("{{url('admjava-auth/form/fProfile')}}");
    }
    
    function loadMenu(){
        $.ajax({
            type: 'GET',
            url: "{{ url('admjava-auth/menu') }}",
            dataType: 'json',
            async:false,
            success:function(result){  
                if(result[0].status){
    ...
    $('.sub-menu').on('click','.a_link',function(e){
        ...
        var url = "{{ url('admjava-auth/form')}}/"+form;
        if(form == "" || form == "-"){
            // alert('Form dilock!');
            return false;
        }else{
