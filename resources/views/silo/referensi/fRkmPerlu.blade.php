...
<!-- LIST DATA -->
<x-list-data judul="Data Pengajuan Perlu diproses" tambah="" :thead="array('No Bukti', 'PP', 'Tanggal', 'Keterangan', 'Komentar', 'Aksi')" :thwidth="array(10,10,15,45,10,10)" :thclass="array('','','','','','text-center')" back="true" />
<!-- END LIST DATA -->
                                ...
                                <table class="table table-bordered table-condensed gridexample input-grid" id="input-barang" data-table="Tab data barang" style="width:100%;table-layout:fixed;word-wrap:break-word;white-space:nowrap">
                                    <thead style="background:#F8F8F8">
                                        <tr>
                                            ...
                                            <th style="width:30%;">NAMA RKM</th>
                                            <th style="width:40%;">DAM</th>
                                            <th style="width:25%;">Kode RKM Akhir</th>
                                        </tr>
                                    </thead>
                                    <tbody></tbody>
                                </table>
...
<script type="text/javascript">
                ...
                if(result.status) { 
                    ...
                    if(result.data_detail.length > 0) { 
                        ...
                        for(var i=0;i<result.data_detail.length;i++) { 
                            ...
                            var idDam = 'dam-ke__'+no
                            var idRkm2 = 'rkm2-ke__'+no

                            html += `
                                <tr class='row-grid'>
                                    <td class='no-grid text-center'>${no}</td>
                                    <td id='${idDesk}'>
                                        <span id='text-${idDesk}' class='tooltip-span'>${data.nama}</span>
                                    </td>
                                    <td id='${idDam}'>
                                        <span id='text-${idDam}' class='tooltip-span'>${data.dam}</span>
                                    </td>
                                    <td id='${idRkm2}'>
                                        <span id='text-${idRkm2}' class='tooltip-span'>${data.kode_rkm_akhir}</span>
                                    </td>
                                </tr>
                            `;
                            no++;
    ...
    function printPreview(id, jenis) {
        $.ajax({
            type: 'GET',
            url: "{{ url('rkap-trans/app-preview') }}",
            dataType: 'json',
